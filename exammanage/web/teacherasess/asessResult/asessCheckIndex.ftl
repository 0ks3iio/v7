	...
	<div class="box-body">
		<div class="filter">
			<div id="importDiv" class="filter-item filter-item-right">
				<div class="filter">
	                <a href="javascript:" class="btn btn-white pull-right" style="margin-bottom:5px;margin-right: 10px" onclick="doExport()">导出</a>
	            </div>
			</div>
		</div>
...
function showList(subjectId){
	...
	var url="${request.contextPath}/teacherasess/asessCheck/list"+parmUrl;
	$("#scoreList").load(url);
}
...
function doExport(){
    ...
    var url='${request.contextPath}/teacherasess/asessCheck/export'+str;
    location.href=url;
}
</script>