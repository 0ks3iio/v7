	...
	<body class="no-skin report-container">
	                                ...
	                                <#list courseList as item>
		                                <div class="mb20">
		                                	<div class="text-center">
		                                		<button class="btn btn-ringblue btn-radius18 mr10">${item.subjectName!}</button>
		                                	</div>
		                                	<div class="report-subtit mt30">平均分分析</div>
		                                	<p class="mt20 font-16 xuankaoShow${item_index}" style="display:none;"><b>考试平均分</b></p>
		                                	<div id="chart1${item_index}" style="height: 300px;"></div>
		                                	<p class="mt20 font-16 xuankaoShow${item_index}" style="display:none;"><b>赋分平均分</b></p>
		                                	<div class="xuankaoShow${item_index}" id="chart11${item_index}" style="height: 300px;display:none;"></div>
		                                	<div class="report-subtit">分数段分析</div>
		                                	<p class="mt20 font-16"><b>班级对比</b></p>
		                                	<div id="chart2${item_index}" style="height: 300px;"></div>
		                                	<p class="mt20 font-16"><b>年级统计</b></p>
		                                	<div id="chart22${item_index}" style="height: 300px;"></div>
		                                	<div class="report-subtit">名次段分析</div>
		                                	<p class="mt20 font-16"><b>班级对比</b></p>
		                                	<div id="chart3${item_index}" style="height: 300px;"></div>
		                                	<p class="mt20 font-16"><b>年级统计</b></p>
		                                	<div id="chart33${item_index}" style="height: 300px;"></div>
		                                </div>
		                            </#list>
                                </#if>
		...
		<script>
			$(function(){
				...
				<#if courseList?exists && courseList?size gt 0>
	                <#list courseList as item>
	                	getChart1('${item.id!}','${item_index}');
	                	getChart3('${item.id!}','${item_index}');
	                </#list>
	            </#if>
			})
			//平均分分析
			function getChart1(subjectId,index){
				...
				$.ajax({
					...
					success: function(json){
						if(json.loadingData11){//赋分平均分
							$(".xuankaoShow"+index).show();
							setChart1("chart11"+index,json.xAxisData,json.loadingData11[0]);
						}else{
							$(".xuankaoShow"+index).hide();
						}
						if(json.loadingData){//考试分平均分
							setChart1("chart1"+index,json.xAxisData,json.loadingData[0]);
						}
						if(json.legendData2){//分数段-班级对比
							setChart2("chart2"+index,json.legendData2,json.xAxisData2,json.loadingData2,json.stackData2);
						}
						if(json.legendData22){//分数段-年级统计
							setChart22("chart22"+index,json.xAxisData22,json.loadingData22);
						}
					}
				});
			}
			//获取名次段
			function getChart3(subjectId,index){
				var examId=$("#examId").val();
				var unitId=$("#unitId").val();
				$.ajax({
					...
					success: function(json){
						if(json.legendData2){//名次段-班级对比
							setChart2("chart3"+index,json.legendData2,json.xAxisData2,json.loadingData2,json.stackData2);
						}
						if(json.loadingData2 && json.loadingData2.length>0){//名次段-年级统计
							setChart22("chart33"+index,json.xAxisData22,json.loadingData22);
						}
					}
				});
			}
					...
					success: function(json){
						...
						if(json.courseList &&json.courseList.length>0 && array && array.length>0){
							for(var i=0;i<array.length;i++){
								subjectComHtml='<li><div class="btn btn-ringblue btn-radius18">';
								...
								setChart4("chart4"+i,json.courseList,array[i].loadingData);
							}
						}
						getChart5();
					}
