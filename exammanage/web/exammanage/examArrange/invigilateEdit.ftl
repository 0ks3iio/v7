...
$("#arrange-commit").on("click",function(){
	...
	// 提交数据
	var options = {
		url : '${request.contextPath}/exammanage/examArrange/invigilateSave'+str,
		dataType : 'json',
		success : function(data){
	 		if(data.success){
	 			layer.closeAll();
				layerTipMsg(data.success,"成功",data.msg);
				var url =  '${request.contextPath}/exammanage/examArrange/invigilateTeacherIndex/page?examId=${examId!}&status=${status!}&subjectId=${subjectId!}';
				$("#showTabDiv").load(url);
	 		}
	 		else{
