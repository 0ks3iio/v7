...
<div class="table-container-header clearfix">
	...
	<input type="checkbox" class="wp wp-switch js-toggleLock" <#if classLock?exists && (classLock.lockState!'0') == '1'>checked="true"</#if>><span class="lbl"></span></label>
</div>
</#if>
			...
			<#list dtoList as dto>
				<tr>
					...
					<td>
						...
						<input type="text" <#if (classLock?exists && (classLock.lockState!'0') == '1') || (hasEditRole == -1)>disabled</#if> class="table-input score_class" name="dtoList[${dto_index}].score" id="score_${dto_index}" vtype = "number" nullable="false"  placeholder="请输入考试成绩" value="${dto.score!}" maxLength="7">
					</td>
					<td><#if dto.toScore?exists>${dto.toScore!}<#elseif dto.score?exists>此班级未设置学分<#else>该学生未录入成绩</#if></td>
					<td>
						<select <#if (classLock?exists && (classLock.lockState!'0') == '1') || (hasEditRole == -1)>disabled</#if> name="dtoList[${dto_index}].scoreStatus" id="scoreStatus_${dto_index}" class="form-control"  notnull="false">
...
<br/>
<div class="row">
	<#if classLock?exists && (classLock.lockState!'0') == '1'>
...
function changeLockStatus(isLock,index){
	var classLockId = '${classLock.id!}';
	url = '${request.contextPath}/exammanage/scoreOptional/lock';
	params = 'classLockId='+classLockId+'&isLock='+isLock;
...
function save(toSubmit,index){
	...
	// 提交数据
	var options = {
		url : '${request.contextPath}/exammanage/scoreOptional/saveAll',
		dataType : 'json',
		success : function(data){
	 		if(data.success){
	 			if(toSubmit){
	 				changeLockStatus('1',index);
	 			}
	 			layer.msg(data.msg, {
					offset: 't',
					time: 2000
