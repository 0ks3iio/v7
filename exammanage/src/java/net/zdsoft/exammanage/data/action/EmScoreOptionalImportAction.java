 ...
 */
@Controller
@RequestMapping("/exammanage/scoreOptional/import")
public class EmScoreOptionalImportAction extends DataImportAction {
    ...
    public String index(EmScoreInfo scoreInfo, ModelMap map) {
        LoginInfo loginInfo = getLoginInfo();
        // 业务名称
        map.put("businessName", "选修课成绩录入");
        // 导入URL
        map.put("businessUrl", "/exammanage/scoreOptional/import/importData");
        // 导入模板
        map.put("templateDownloadUrl", "/exammanage/scoreOptional/import/template?teachClassId=" + scoreInfo.getTeachClassId());
        // 导入对象
        map.put("objectName", "");
        // 导入说明
        map.put("description", "");
        ...
        //模板校验
        map.put("validateUrl", "/exammanage/scoreOptional/import/validate");
        // 导入说明
        ...
        return "/exammanage/optionalScoreInfo/optionalScoreImport.ftl";
    }
    ...
    public String dataImport(String filePath, String params) {
        ...
        if (Constant.IS_TRUE_Str.equals(teachClass.getIsMerge())) {
            ...
            scoreInfList = emScoreInfoService.findOptionalCourseScore(unitId, classIds);
        } else {
...
//        Integer credit = course.getInitCredit();
        //从数据库中获取数据
        Map<String, EmScoreInfo> scoreInfMap = EntityUtils.getMap(scoreInfList, "studentId");

        Set<String> stuCodes = new HashSet<>();
        List<EmScoreInfo> forSaveList = new ArrayList<>();
        int successCount = 0;
        int i = 2;
        for (String[] arr : datas) {
            ...
            if ((forSave = scoreInfMap.get(stuMap.get(stuCode).getId())) == null) {
                ...
                forSave.setInputType(ExammanageConstants.ACHI_SCORE);
        ...
        if (CollectionUtils.isNotEmpty(forSaveList)) {
            emScoreInfoService.saveAllEntitys(forSaveList.toArray(new EmScoreInfo[0]));
        }
