	<p class="tree-name no-margin">
		<b>展示内容</b>
		<#if fullObj.sourceType=='01'>
		<#if fullObj.status != 1><button class="btn btn-blue btn-30" onclick="fullObjEdit('${fullObj.id!}')">编辑</button>
		<#else>
		<span class="lock-switch clearfix">
			锁定:
			<label>
				<input id="lockScreenOne" <#if fullObj.lockScreen>checked</#if> class="wp wp-switch js-openNoticeClass" type="checkbox">
				<span class="lbl"></span>
			</label>
		</span>
		</#if>
		<button class="btn btn-default btn-30" onclick="fullObjDelete('${fullObj.id!}')">删除</button>
		</#if>
	</p>
	...
	<div class="group-content">
		<p class="group-title no-margin">
			...
			<span>来源：<#if fullObj.type=='01'>全屏公告（由校级管理员发布）<#else><#if fullObj.sourceType=='02'>校级多媒体（由校级管理员发布）<#else>班级多媒体</#if></#if></span>
		</p>
		...
		<div class="card-list card-list-large no-margin scroll-height2 clearfix js-show">													
			<!--图片浏览-->
			<#if fullObj.type=='03'&&object?exists&&object.attachments?size gt 0>
			...
			<!--视频浏览-->
			<#if fullObj.type=='04'&&object?exists&&object.attachments?size gt 0>
				<div class="media-part block">
			        <#list object.attachments as item>
					<div class="card-item">
						<div class="card-content">
			...
			<!--PPT浏览-->
			<#if fullObj.type=='05'&&object?exists&&object.attachments?size gt 0>
			...
			<!--全屏公告-->
			<#if fullObj.type=='01'&&object?exists>
				<div class="media-part block">
			...
			<#if fullObj.type!='01'&&(!object?exists || object.attachments?size == 0)>
...
function fullObjLock(lock){
	$.ajax({
		url:'${request.contextPath}/eclasscard/standard/fullscreen/lockobj',
		data: {'id':"${fullObj.id!}",'lock':lock},
		type:'post',
