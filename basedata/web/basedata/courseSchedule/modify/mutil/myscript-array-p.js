...
var movePeriods_tip = '走班时间或其他年级课程，无法操作';
$(function(){
	//框架结构
	...
	$('.content .syllabus-table td .item').on('click','.replace a',function(e){
		...
		if(left_weekType==3 || right_weekType==3){
			autoTips("课程不是单双周上课不能交换");
			return;
		}

		var code1 = left_classId+"_"+left_subjectId+","+right_classId+"_"+right_subjectId;
		var $sub = $(".syllabus-from li[data-user='"+code1+"']");
		if($sub.length > 0){
			var weekType = $sub.find("[name=\"weekType\"]").val();
			if(weekType && weekType != "3"){
				autoTips("单双周已固定 不能交换");
				return ;
			}
		}
		...
		param += "&period=" + period+"&type=T";
		
		var arrayId = indexId;
		var url = contextPath + "/"+url_sub_sys+"/scheduleModify/"+arrayId+"/swapWeekType";
		$.ajax({
			url: url,
			data:param,
			type:"POST",
				...
				if(!$(this).hasClass('item-lock')){
					...
					if(!$(this).hasClass('item-sel')){
						...
						if(sel==0){
							...
							if(sub_sys != "BASE"){
								var classId_now = $(this).find("[name='classId']").val();
								var classType = $(this).find("[name='classType']").val();
								if(classType != 4){
									freshXzbPeriods(classId_now);
								}else{
									freshZhbPeriods();
								}
							}
							...
							//处理 使用这个 场地的 行政班 的 虚拟课程 时间点 
							var classId = $(this).find("[name='classId']").val();
							insVirtualNoTime(classId);
...
//							if(!f || !g) return;
							...
							var classId_old = $(".content tbody td .item-sel").find("[name='classId']").val();
							var classType_old = $(".content tbody td .item-sel").find("[name='classType']").val();
							...
							var classId_now = $(this).find("[name='classId']").val();
							var classType_now = $(this).find("[name='classType']").val();
							...
							if($(".content tbody td .item-sel").find(".s-d-week").length > 0){
								...
								var classId_old2 = $(".content tbody td .item-sel").find("[name='classId']:eq(1)").val();
								var classType_old2 = $(".content tbody td .item-sel").find("[name='classType']:eq(1)").val();
								
								param += "&leftSchedules[1].id="+id_old2;
								param += "&leftSchedules[1].placeId="+placeId_old2;
								param += "&leftSchedules[1].classId="+classId_old2;
								param += "&leftSchedules[1].teacherId="+teacherId_old2;
								param += "&leftSchedules[1].weekType="+weekType_old2;
								param += "&leftSchedules[1].classType="+classType_old2;
							...
							if($(this).find(".s-d-week").length > 0){
								...
								var classId_now2 = $(this).find("[name='classId']:eq(1)").val();
								var classType_now2 = $(this).find("[name='classType']:eq(1)").val();
								
								param += "&rightSchedules[1].id="+id_now2;
								param += "&rightSchedules[1].placeId="+placeId_now2;
								param += "&rightSchedules[1].classId="+classId_now2;
								param += "&rightSchedules[1].teacherId="+teacherId_now2;
								param += "&rightSchedules[1].weekType="+weekType_now2;
								param += "&rightSchedules[1].classType="+classType_now2;
...
//							return ;
							var arrayId = indexId;
							var url = contextPath + "/"+url_sub_sys+"/scheduleModify/"+arrayId+"/swapPlaceCourse";
							var obj = this;
							$.ajax({
								url: url,
								data:param,
...
//					console.log("timestr = "+timestr+(timestr == theTime));
	...
	function getTeSchedule(teacherIds,fix){
		...
		var arrayId = indexId;
		var url = contextPath + "/"+url_sub_sys+"/scheduleModify/mutil/"+arrayId+"/teacherSchedule?"+arrayIdParams;
		var searchWeek = $(".ui-select-value").val();
		console.log("classIds: "+teacherIds);
		//return;
		$.ajax({
			url: url,
	...
	function insTeaSchedule(teacherIds, scheduleObj){
		...
		if(teacherIds.length > 1){
			...
			spantitle = '<span class="tab switch active" data-tid="'+teacherIds[0]+'">\
								...
								<span class="mr-5 ml-5">'+tname1+'</span>课程表\
							</span>\
							<span class="tab" data-tid="'+teacherIds[1]+'">\
								<a class="ml-5" href="javascript:void(0);"><img style="position: relative;top: 2px;" src="/static/images/array/fullscreen.png"></a>\
								<span class="mr-5 ml-5">'+tname2+'</span>课程表\
							</span>';
			
			$("#teacherTable1").prev("p").html(spantitle);
		}else{
			var tname = teacherNameMap[teacherIds[0]];
			spantitle = '<span class="switch" style="float:left;width:100%;" data-tid="'+teacherIds[0]+'"><span class="mr-5 ml-5 " >'+tname+'</span> 课程表</span>';
		}
		$("#teacherTable1").prev("p").html(spantitle);
	...
	// 添加科目禁排时间和 教师禁排时间
	function insVirtualNoTime(cid){
		var notimes = classVirtualTimeMap[cid];
		if(!notimes){
			return;
		}
		$('.content .syllabus-table td .item').not('.item-lock').each(function(){
			var timestr = $(this).parents("td").attr("data-time");
			if(notimes.indexOf(timestr) > -1){
				//$(this).addClass('movePeriod').addClass('item-lock');
				$(this).addClass('item-conflict');
			};
		});
	} 
	...
	function showTeacherNoTime(obj){
		...
		var classId1 = $(obj).find("[name='classId']:eq(0)").val();
		...
		$("#allTable tbody td .item:not(.item-lock)").each(function(){
			...
			// 选定点有两节课（单双周）时的情况
			// 选定点 在此处的 禁排情况
			var f = checkConflict(this,t,weekType,subjectType1,teacherId1,subjectId1,classOldIdMap[classId1]);
			if(f){
				return;
			}
			...
			if(id_len ==2){
				// 第二个
				f = checkConflict(this,t,weekType2,subjectType2,teacherId2,subjectId2,classOldIdMap[classId2]);
				if(f){
					return;
				}
			}
			...
			var weekTypes = parseInt($(this).find("input[name='weekType']").val());
			var weekTypes2 = parseInt($(this).find("input[name='weekType']:eq(1)").val());
			var cid = parseInt($(this).find("input[name='classId']").val());
			var cid2 = parseInt($(this).find("input[name='classId']:eq(1)").val());
			
			if(!tid && !tid2){
				return;
			}
			
			f = checkConflict(this,time,weekTypes,subType,tid,subId,classOldIdMap[cid]);
			if(f){
				return;
			}
			
			if(len == 2){
				f = checkConflict(this,time,weekTypes2,subType2,tid2,subId2,classOldIdMap[cid2]);
				if(f){
					return;
				}
			}
	...
	function checkConflict(obj,time,weekType,subType,tid,subId,oldCid){
		//debugger
		var tcs = teacherScheduleMap[tid];
		var gp = combineRes[oldCid+"-"+subId+"-"+subType];
		if(!tcs){
			return false;
		}
...
//		var $str2='<span class="course" style="height:'+span_h+';line-height:'+span_h+'">'+into_course+'</span><span class="name" style="height:'+span_h+';line-height:'+span_h+'">'+into_name+'</span><span class="room" style="height:'+span_h+';line-height:'+span_h+'">'+into_room+'</span>';
	...
	function ioSwap2(obj,e){
		...
		//移出已排课程
		$(obj).clone().addClass("item-can").css({top:into_top,left:into_left}).prependTo($('body'));
	...
	$('.syllabus-from').on('click','li',function(){
		...
		if(data_lock == 0){
			...
			if(!$(this).hasClass('over')){
				...
				showTeacherNoTime(this);
				// 行政班 虚拟课程
				var cid2 = $(this).find("[name='classId']").val();
				insVirtualNoTime(cid2);
				//添加课表可操作状态
...
//						console.log(notime);
				...
				var inner_classId = $(this).find("[name='classId']").val();
				...
				var inner_classId2 = $(this).find("[name='classId']:eq(1)").val();
...
//				var obj = this;
				...
				$('#allTable').off('click').on('click','span.item',function(e){
					...
					if($(this).hasClass("item-conflict") && !inner_subjectId2){
						autoTips('教师有课或禁排时间，不能排课');
						return;
					}
					...
					if(!$(this).hasClass('item-lock')){
						var data_sel=parseInt($('body').attr('data-sel'));
						if(data_sel==1){
							var cln_use2 = $(this).attr("data-user");
							if(cln_use2 && cln_use2 == cln_user){
								autoTips('同一门课程不能移动');
							}else{
								isSaving = true;
								...
								var outer_classId1 = $(this).find("[name='classId']").val();
								...
								var outer_classId2 = $(this).find("[name='classId']:eq(1)").val();
...
//								var classId = objId;
								...
								if(subjId && subjId!="0"){ 
									...
									param += "&leftSchedules[0].classId="+outer_classId1;
									param += "&leftSchedules[0].subjectType="+outer_subjectType1;
									leftCids.push(outer_classId1);
									if($(".int-from").find(".s-d-week").length > 0){
										...
										param += "&leftSchedules[1].classId="+outer_classId2;
										param += "&leftSchedules[1].subjectType="+outer_subjectType2;
										leftCids.push(outer_classId2);
									}
								}
								...
								param += "&rightSchedules[0].classId="+ inner_classId;
								param += "&rightSchedules[0].weekType="+inner_weekType;
								rightCids.push(inner_classId);
								if(s_d_week){
									...
									param += "&rightSchedules[1].classId="+inner_classId2;
									param += "&rightSchedules[1].weekType="+inner_weekType2;
									rightCids.push(inner_classId2);
								}
...
//											autoTips("操作成功");
											for(x in leftCids){
												updateClassSurplus(leftCids[x],1);
											}
											for(x in rightCids){
												updateClassSurplus(rightCids[x],-1);
											}
										}else{
											autoTips(""+data.msg);
											isSaving = false;
			...
			var outer_classId1 = $(".int-from").find("[name='classId']").val();
			var outer_subjectId1 = $(".int-from").find("[name='subjectId']").val();
			var outer_subjectType1 = $(".int-from").find("[name='subjectType']").val();
			...
			var outer_classId2 = $(".int-from").find("[name='classId']:eq(1)").val();
			var outer_subjectId2 = $(".int-from").find("[name='subjectId']:eq(1)").val();
			var outer_subjectType2 = $(".int-from").find("[name='subjectType']:eq(1)").val();
			
			
			if(outer_id2){
				var data_c = outer_classId1+"_"+outer_subjectId1+","+outer_classId2+"_"+outer_subjectId2;
				var data_c2 = outer_classId2+"_"+outer_subjectId2+","+outer_classId1+"_"+outer_subjectId1;
				var len = $(".syllabus-from li[data-user='"+data_c+"']").length;
				var len2 = $(".syllabus-from li[data-user='"+data_c2+"']").length;
				if(len <= 0 && len2 <= 0){
					isSaving = false;
					autoTips("不能删除教学班课程");
					return;
...
function insScheduleOnly(id,schedules){
	...
	for(i in schedules){
		...
		var teacherName = x.teacherName?x.teacherName:"";
