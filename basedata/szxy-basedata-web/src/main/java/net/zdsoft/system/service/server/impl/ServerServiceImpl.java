...
public class ServerServiceImpl extends BaseServiceImpl<Server, Integer> implements ServerService {
    ...
    @Override
    public List<Server> getServersByCodes(String[] codes) {
        if (codes== null || codes.length == 0) {
            return Collections.emptyList();
        }
        return serverDao.getServersByCodes(codes);
    }
    ...
    public List<Server> findAppsByDevIds(String[] devIds) {
        return serverDao.findAppsByDevIds(devIds);
    }
    ...
    public Server addApp(Server server) {
        ...
        //设置
        if (server.getOrderId() == null) {
            server.setOrderId(serverDao.getMaxOrderId() + 1);
        }
    ...
    public void updateEnableServer(String[] codes) {
        ...
        sysPlatformModelDao.stopAllModel();
...
//        serverDao.updateServerClassToInner(EntityUtils.toArray(EntityUtils.<String, Server>getList(innerServerList, "code"), String.class));
        ...
        sysPlatformModelDao.activeModels(subSystemSet.toArray(new Integer[0]));
    }
...
//            client.addServer(serverDto);
    ...
    public Server findBySubId(Integer subId) {
        return RedisUtils.getObject(RedisUtils.getKey(Server.class, String.valueOf(subId)),
                RedisUtils.TIME_FIVE_MINUTES, Server.class, () -> serverDao.findBySubId(subId));
    }

    @Override
    public List<Server> findBySubIds(Integer[] subIds) {
        if (subIds == null || subIds.length == 0) {
            return Collections.emptyList();
        }
        if (subIds.length == 1) {
            return Lists.newArrayList(findBySubId(subIds[0]));
        }
        return serverDao.findBySubIds(subIds);
    }
