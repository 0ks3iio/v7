...
<script type="text/javascript">
	...
	function addRichText2Report(){
		 $.ajax({
		            url:"${request.contextPath}/bigdata/multireport/component/richText/new",
		            data:{
		              'reportId':'${reportId!}'		           
		            },
		            type:"post",
		            ...
		            success:function(data){
		            	layer.closeAll();
				 		if(!data.success){
	         				...
	         				$('.page-content').load('${request.contextPath}/bigdata/multireport/design?type=7&reportId=${reportId!}');
		    			}
		          },
	...
	function reportComponentSet(businessType,reportId,componentId){
		...
		obj.load("${request.contextPath}/bigdata/multireport/component/set?type=7&reportId="+reportId+"&componentId="+componentId+"&businessType="+businessType);
		return false;
	}
	...
	function componentSet(reportId,componentId){
			...
			$("#boardComponentEditDiv").empty().load("${request.contextPath}/bigdata/multireport/component/set?type=7&reportId="+reportId+"&componentId="+componentId);
			return false;
		}
	...
	$(document).ready(function(){
		<#if reportDetailList?exists&&reportDetailList?size gt 0>
			<#list reportDetailList as component>
				  wangEditorZindex--;
				  $("#report-detail-${component.id!}").load("${request.contextPath}/bigdata/multireport/component/detail?type=7&componentId=${component.id!}&zIndex="+wangEditorZindex);		
			</#list>
		</#if>
	});
</script>