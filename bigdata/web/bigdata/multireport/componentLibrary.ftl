...
<script type="text/javascript">
	...
	function addComponent2Report(){
		...
		if(type=="6" ){
		 ...
		 var options = {
			url:"${request.contextPath}/bigdata/multireport/component/save",
			dataType : 'json',
			success : function(data){
				layer.closeAll();
		 		if(!data.success){
		 			...
		 			$('.page-content').load('${request.contextPath}/bigdata/multireport/design?type='+type+'&reportId='+$("#reportId").val());
    			}
			},
			clearForm : false,
			resetForm : false,
