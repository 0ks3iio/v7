...
<script>
	function editKylinJob(id,name){
		router.go({
	        path: 'bigdata/calculate/kylin/edit?cubeName='+name,
	        name:'编辑',
	        level: 3
	    }, function () {
		    var url = "${request.contextPath}/bigdata/calculate/kylin/edit?cubeName="+name;
			$("#contentDiv").load(url);
	    });
	}
	...
	function execKylinJob(id){
		 $.ajax({
	            ...
	            success:function(data){
	    			...
	    			$("#contentDiv").load("${request.contextPath}/bigdata/calculate/offline/list?calculateType=2");
	          }
	    });
	}
