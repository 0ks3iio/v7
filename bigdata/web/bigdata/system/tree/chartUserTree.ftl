...
<script>
	...
	function onUserTreeCheck(event, treeId, treeNode){
		if(treeNode.type == "dept"){
			...
			if(treeNode.checked){
				...
				var str='<div id="'+treeNode.id+'" class="teacher-user-tree">'+treeNode.name+'<i class="fa fa-times-circle"></i></div>';
	...
	function loadOldUsers(){
		 ...
		 for (var i = 0; i < jsondata.length; i++){
		 	...
		 	var str='<div id="'+userId+'" class="teacher-user-tree">'+userName+'<i class="fa fa-times-circle"></i></div>';
   			$('.choose-item').append(str);
   			num++;
		 }
		 $('.choose-num span').text(num);
