...
<#if  userAuthList?exists &&userAuthList?size gt 0>
<table class="tables">
	<thead>
		<tr>
			<th>用户名</th>
...
<script>
	...
	function addModuleUser(id){
		$.ajax({
	            url:'${request.contextPath}/bigdata/userauth/addUser',
	            data:{
		           'userId':id
	            },
	            type:"post",
	...
	function searchModuleUser() {
	    ...
	    var url = "${request.contextPath}/bigdata/userauth/userSelect?username=" + username + "&realname=" + realname;
	    $("#contentDiv").load(url);
	}
</script>