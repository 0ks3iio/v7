...
<input type="hidden" name="etlType" value="${python.etlType!}">
<input type="hidden" name="jobType" value="python">
<input type="hidden" name="targetType" value="table">
    <div class="form-horizontal">
        <div class="form-group">
            ...
            <div class="col-sm-6">
                <input type="text" name="name" id="name" class="form-control" nullable="false" maxLength="36" value="${python.name!}">
            </div>
        </div>
            ...
            <div class="col-sm-6">
                <select id="nodeId" class="form-control" nullable="true" name="nodeId">
                        <#list nodeList as node>
                            <option value="${node.id!}" <#if node.id! == python.nodeId!>selected="selected"</#if>>${node.name!}</option>
                        </#list>
                </select>
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-6">
                <select id="sourceType" class="form-control" name="sourceType" onchange="changeSourceType()">
                    <option value="table" <#if python.sourceType?default('table') == 'table'>selected="selected"</#if>>
                        表
                    </option>
                    <option value="app" <#if python.sourceType?default('table') == 'app'>selected="selected"</#if>>
                        应用
                    </option>
                </select>
            </div>
            ...
            <div class="col-sm-6">
                <select id="targetId" class="form-control" nullable="true" name="targetId">
                    <option value="">---请选择---</option>
                        <#list mdList as md>
                            <option value="${md.id!}" <#if md.id! == python.targetId!>selected="selected"</#if>>${md.name!}</option>
                        </#list>
                </select>
            </div>
        </div>
        ...
        <div class="form-group">
			<@upload.fileUpload businessKey="python" contextPath="${request.contextPath}" resourceUrl="${resourceUrl!}" size="1" fileNumLimit="1" handler="loadFile">
                <label class="col-sm-2 control-label no-padding-right"><font style="color:red;">*</font>调度文件：</label>
                <div class="col-sm-6">
                    <a class="file js-addFiles">上传文件</a>
                    <p class="js-file-content" id="showFileName">${python.fileName!}</p>
                    <input type="hidden" id="python-path" value="">
                    <input type="hidden" name="path" id="path" value="${python.path!}">
                    <input type="hidden" name="fileName" id="fileName" value="${python.fileName!}">
                </div>
            </@upload.fileUpload>
        </div>
        <div class="form-group">
            ...
            <div class="col-sm-6">
                <label class="switch-label">
                    <input <#if python.isSchedule?default(0) ==1>checked="true"</#if> class="wp wp-switch js-isSchedule" type="checkbox">
                    <span class="lbl"></span>
                </label>
                <input type="hidden" id="isSchedule" name="isSchedule" value="${python.isSchedule?default(0)}">
            </div>
        </div>
        <div class="form-group form-shechule-param <#if python.isSchedule?default(0) ==0>hidden</#if>" >
            <label class="col-sm-2 control-label no-padding-right">定时执行参数：</label>
            <div class="col-sm-6">
                <input type="text" name="scheduleParam" id="scheduleParam" class="form-control" nullable="true" maxLength="100" value="${python.scheduleParam!}">
            </div>
            <button class="btn btn-blue" type="button" id="paramBtn">参数配置实例</button>
        </div>
        <!--
		<div class="form-group form-shechule-has-param <#if python.isSchedule?default(0) ==1>hidden</#if>" >
			<label class="col-sm-2 control-label no-padding-right">是否包含参数：</label>
			<div class="col-sm-6">
				<label class="switch-label">
					<input <#if python.hasParam?default(0) ==1>checked="true"</#if> class="wp wp-switch js-hasParam" type="checkbox">
					<span class="lbl"></span>
				</label>
				<input type="hidden" id="hasParam" name="hasParam" value="${python.hasParam?default(0)}">
			</div>
		</div>
		-->
        <div class="form-group">
            ...
            <div class="col-sm-6">
                <input type="text" name="remark" id="remark" class="form-control" nullable="true" maxLength="200" value="${python.remark!}">
            </div>
        </div>
        ...
        <div class="form-group" style="display: none">
            ...
            <div class="col-sm-6">
                <textarea id="flowChartJson" name="flowChartJson">${python.flowChartJson!}</textarea>
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-6" >
                <button type="button" class="btn btn-long btn-blue js-added" id="pythonSaveBtn">&nbsp;保存&nbsp;</button>
            </div>
        </div>
    </div>
    <div id="paramDiv"></div>
...
<script>
    ...
    $("#pythonSaveBtn").on("click",function(){
        if(isSubmit){
            return;
        }
        isSubmit = true;
        ...
        var options = {
            url : "${request.contextPath}/bigdata/etl/python/save",
            dataType : 'json',
    ...
    function changeSourceType(){
        ...
        if (sourceType == 'app') {
         ...
         <#if  appList?exists &&appList?size gt 0>
             <#list appList as app>
                $("#sourceId").append("<option value='${app.id!}'<#if app.id! == python.sourceId!>selected=\"selected\"</#if>>${app.name!}</option>");
             </#list>
         </#if>
        } else {
            $("#sourceId").empty();
         ...
         <#if  mdList?exists &&mdList?size gt 0>
             <#list mdList as md>
                $("#sourceId").append("<option value='${md.id!}'<#if md.id! == python.sourceId!>selected=\"selected\"</#if>>${md.name!}</option>");
             </#list>
         </#if>
        }
    }
    ...
    $(function () {
        changeSourceType();
        $('body').load(init());
        <#if python.flowChartJson?exists && python.flowChartJson?length gt 0>
        loadFlowChartJson();
        </#if>
    })
</script>