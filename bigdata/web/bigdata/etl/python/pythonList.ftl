<div class="table-show">
    <div class="filter-made mb-10">
        <div class="filter-item filter-item-right clearfix">
            <button class="btn btn-lightblue" onclick="addPythonJob();">新增</button>
        </div>
    </div>
    <table class="tables tables-border no-margin">
        ...
        <tbody>
		<#if pythonList?exists&&pythonList?size gt 0>
            <#list pythonList as python>
			<tr>
                <td>${python.name!}</td>
                <td><#if python.isSchedule?default(0) ==1>是<#else>否</#if></td>
                <td>${python.lastCommitTime!}</td>
                <td>
						<#if python.lastCommitState?default(0) ==1>
                            <span class="success-left">成功</span>
                        <#elseif python.lastCommitState?default(0) ==2>
                ...
                <td>
					<#if python.lastCommitState?default(3) ==2>
						<a class="look-over" href="javascript:void(0);"  onclick="viewErrorLog('${python.lastCommitLogId!}')">
                            错误日志</a><span class="tables-line">|</span>
                    </#if>
                    <a href="javascript:;" class="look-over" onclick="execPythonJob('${python.id!}','${python.hasParam?default(0)}')">执行</a><span class="tables-line">|</span>
                    <a class="look-over" href="javascript:void(0);" onclick="editPythonJob('${python.id!}')">编辑</a><span class="tables-line">|</span>
                    <a class="remove" href="javascript:void(0);" onclick="deletePythonJob('${python.id!}','${python.name!}')">删除</a><span class="tables-line">|</span>
                    <a id="log-${python.id!}-button"  href="javascript:;" class="look-over js-log-show" onclick="loadPythonLogList('${python.id!}');" >日志</a>
                </td>
            </tr>
            </#list>
        <#else>
...
<script>
    function addPythonJob(){
        router.go({
            path: '/bigdata/etl/python/edit',
            name:'新增',
            level: 3
        }, function () {
            var url =  '${request.contextPath}/bigdata/etl/python/edit';
            $("#contentDiv").load(url);
        });
    }

    function editPythonJob(id){
        router.go({
            path: '/bigdata/etl/python/edit?id='+id,
            name: '编辑',
            level: 3
        }, function () {
            var url =  '${request.contextPath}/bigdata/etl/python/edit?id='+id;
            $("#contentDiv").load(url);
        });
    }

    function loadPythonLogList(id){
    ...
    function execPythonJob(id,hasParam){
        if(hasParam =="0"){
            $.ajax({
                ...
                success:function(data){
                    ...
                    var url =  "${request.contextPath}/bigdata/etl/list?etlType=7";
                    $("#contentDiv").load(url);
                }
            });
        }else{
    ...
    function deletePythonJob(id,name){
        showConfirmTips('prompt',"提示1","您确定要删除"+name+"吗？",function(){
            $.ajax({
                url:"${request.contextPath}/bigdata/etl/python/delete",
                data:{
                    'id':id
                },
                type:"post",
