<div class="table-show">
	...
	<table class="tables tables-border no-margin">
		...
		<#if flinkList?exists&&flinkList?size gt 0>
			...
			<tr>
				<td>${flink.name!}</td>
				<td><#if flink.isSchedule?default(0) ==1>是<#else>否</#if></td>
				<td>${flink.lastCommitTime!}</td>
				<td>
						<#if flink.lastCommitState?default(0) ==1>
							<span class="success-left">成功</span>
						<#elseif flink.lastCommitState?default(0) ==2>
				...
				<td>
					<#if flink.lastCommitState?default(3) ==2>
						<a class="look-over" href="javascript:void(0);"  onclick="viewErrorLog('${flink.lastCommitLogId!}')">
						错误日志</a><span class="tables-line">|</span>
					</#if>
                    <a href="javascript:;" class="look-over" onclick="execFlinkJob('${flink.id!}','${flink.hasParam?default(0)}')">执行</a><span class="tables-line">|</span>
					<a class="look-over" href="javascript:void(0);" onclick="editFlinkJob('${flink.id!}')">编辑</a><span class="tables-line">|</span>
					<a class="remove" href="javascript:void(0);" onclick="deleteFlinkJob('${flink.id!}','${flink.name!}')">删除</a><span class="tables-line">|</span>
					<a id="log-${flink.id!}-button"  href="javascript:;" class="look-over js-log-show" onclick="loadFlinkLogList('${flink.id!}');" >日志</a>
				</td>
			</tr>
			 </#list>
	 <#else>
...
<script>
	function addFlinkJob(){
		router.go({
	        path: '/bigdata/etl/flink/edit',
	        name:'新增',
	        level: 3
	    }, function () {
		    var url =  '${request.contextPath}/bigdata/etl/flink/edit';
			$("#contentDiv").load(url);
	    });
	}
	...
	function editFlinkJob(id){
		router.go({
	        path: '/bigdata/etl/flink/edit?id='+id,
	        name: '编辑',
	        level: 3
	    }, function () {
		    var url =  '${request.contextPath}/bigdata/etl/flink/edit?id='+id;
			$("#contentDiv").load(url);
	    });
	}
	
	function loadFlinkLogList(id){
		...
		if(hasParam =="0"){
			 $.ajax({
		            ...
		            success:function(data){
		    			...
		    			var url =  "${request.contextPath}/bigdata/etl/list?etlType=6";
						$("#contentDiv").load(url);	
		          }
		    });
	    }else{
	...
	function deleteFlinkJob(id,name){
			showConfirmTips('prompt',"提示","您确定要删除"+name+"吗？",function(){
			 $.ajax({
		            url:"${request.contextPath}/bigdata/etl/flink/delete",
		            data:{
		              'id':id
		            },
		            type:"post",
