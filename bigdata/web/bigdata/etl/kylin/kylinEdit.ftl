...
<input type="hidden" name="jobCode" value="${kylinCube.name!}">
    <div class="form-horizontal">
		...
		<div class="form-group" style="display: none">
			...
			<div class="col-sm-6">
				<textarea id="flowChartJson" name="flowChartJson">${kylin.flowChartJson!}</textarea>
			</div>
		</div>
...
<script>
	...
	$("#saveKylinBtn").on("click",function(){
		...
		var options = {
				url : "${request.contextPath}/bigdata/etl/kylin/save",
				dataType : 'json',
	...
	$(function () {
		$('body').load(init());
		<#if kylin.flowChartJson?exists && kylin.flowChartJson?length gt 0>
		loadFlowChartJson();
		</#if>
	})
</script>