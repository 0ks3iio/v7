<form id="metadataForm">
    <div class="form-horizontal form-made">
        ...
        <div class="form-group">
            <label class="col-sm-3 control-label no-padding-right"><font style="color:red;">*</font>是否主键：</label>
            <div class="col-sm-7">
                <label class="choice">
                    <input type="radio" <#if field.isPrimaryKey?default(0) == 1>checked="checked"</#if>
                           name="isPrimaryKey" value="1" onchange="changeIsPrimaryKey('1')">
                    <span class="choice-name">是</span>
                </label>
                <label class="choice">
                    <input type="radio" <#if field.isPrimaryKey?default(0) == 0>checked="checked"</#if>
                           name="isPrimaryKey" value="0" onchange="changeIsPrimaryKey('0')">
                    <span class="choice-name">否</span>
                </label>
            </div>
        </div>

        <div class="form-group <#if field.isPrimaryKey?default(0) == 0>hide</#if>" id="primaryTypeDiv">
            <label class="col-sm-3 control-label no-padding-right"><font style="color:red;">*</font>主键类型：</label>
            <div class="col-sm-7">
                <label class="choice">
                    <input type="radio" <#if field.primaryType?default(1) == 1>checked="checked"</#if>
                           name="primaryType" value="1" onchange="changePrimaryType('1')">
                    <span class="choice-name">int自增</span>
                </label>
                <label class="choice">
                    <input type="radio" <#if field.primaryType?default(1) == 2>checked="checked"</#if>
                           name="primaryType" value="2" onchange="changePrimaryType('2')">
        ...
        <#if dbType?default('mysql') == 'mysql'>
            <div class="form-group <#if field.isPrimaryKey?default(0) == 1 && field.primaryType?default(1) == 1>hide</#if>">
                ...
                <#if dbType?default('mysql') == 'mysql'>
                    <div class="col-sm-7">
                        <select id="databaseTypeSelect" name="columnType" class="form-control">
                            <option <#if field.columnType?default('varchar') == 'varchar'>selected="selected"</#if>value="varchar">
                                varchar
                            </option>
                            <option <#if field.columnType?default('varchar') == 'char'>selected="selected"</#if>value="char">
                                char
                            </option>
                            <option <#if field.columnType?default('varchar') == 'int'>selected="selected"</#if> value="int">
                                int
                            </option>
                            <option <#if field.columnType?default('varchar') == 'bigint'>selected="selected"</#if>value="bigint">
                                bigint
                            </option>
                            <option <#if field.columnType?default('varchar') == 'float'>selected="selected"</#if>value="float">
                                float
                            </option>
                            <option <#if field.columnType?default('varchar') == 'double'>selected="selected"</#if>value="double">
                                double
                            </option>
                            <option <#if field.columnType?default('varchar') == 'date'>selected="selected"</#if> value="date">
                                date
                            </option>
                            <option <#if field.columnType?default('varchar') == 'time'>selected="selected"</#if> value="time">
                                time
                            </option>
                            <option <#if field.columnType?default('varchar') == 'datetime'>selected="selected"</#if>
                                    value="datetime">datetime
                            </option>
                            <option <#if field.columnType?default('varchar') == 'timestamp'>selected="selected"</#if>
                                    value="timestamp">timestamp
                            </option>
                        </select>
                    </div>
                <#elseif dbType?default('mysql') == 'hbase'>
                    <div class="col-sm-7">
                        <select id="databaseTypeSelect" name="columnType" class="form-control">
