...
<script type="text/javascript">
		function componentSet(reportId,componentId){
			...
			$("#boardComponentEditDiv").empty().load("${request.contextPath}/bigdata/data/analyse/component/set?type=6&reportId="+reportId+"&componentId="+componentId);
			return false;
		}
			            ...
			            success:function(data){
			            	layer.closeAll();
					 		if(!data.success){
	         					...
	         					$('.page-content').load('${request.contextPath}/bigdata/data/analyse/design?type=6&reportId=${reportId!}');
			    			}
			          },
    	...
    	$(document).ready(function(){
			<#if reportDetailList?exists&&reportDetailList?size gt 0>
				<#list reportDetailList as component>
					  $("#board-detail-${component.id!}").load("${request.contextPath}/bigdata/data/analyse/component/detail?type=6&componentId=${component.id!}");
				</#list>
			</#if>
		});
</script>