...
<script type="text/javascript">
    ...
    $(function () {
        ...
        $(".fileParent").on("change", '.file', function () {
            ...
            if (this && this.files && this.files[0]) {
                ...
                var str = '<div class="every-file" style="height:148;line-height:148px;">\
				        		<img src="${request.contextPath}/static/bigdata/images/csv-icon-grey.png" class="pos-left"/>\
                ...
                $('#upFile').hide();
                $.ajax({
                    url: '${request.contextPath}/bigdata/nosqlDatasource/previewCsvFile',　　　　　　　　　　//上传地址
                    type: 'POST',
                    cache: false,
        ...
        $('#saveBtn').on('click', function () {
            ...
            if ($('#csvName').val().replace(/[\u0391-\uFFE5]/g,"aa").length > 50) {
                $('.page-content').scrollTop(0);
                isSubmit = false;
                layer.tips("名称长度不能超过50", "#csvName", {
                    tipsMore: true,
                    tips: 3
                });
                return;
            ...
            var options = {
                url: "${request.contextPath}/bigdata/nosqlDatasource/saveFile?type=csv",
                dataType: 'json',
                success: function (data) {
                    if (!data.success) {
