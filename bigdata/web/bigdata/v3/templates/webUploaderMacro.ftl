...
<#macro picUpload  businessKey="" contextPath="" resourceUrl="" size="2" extensions="gif,jpg,jpeg,bmp,png" mimeTypes="image/gif,image/jpeg,image/jpg,image/png" fileNumLimit="1" tips="" handler="defaultHandler">
	<#nested>
	<div class="layer layer-addPhotos layer-addFiles height-1of1 over-y layui-layer-wrap">
		...
		<div class="file-default-box">
			...
			<div class="layer-footer text-right">
				<a id="filePicker" class="btn btn-white filePicker2 no-padding-top">选择文件</a>
				<button id="picUploadBtn" class="btn btn-blue" type="button">开始上传</button>
			</div>
		</div>
	</div>
	<script type="text/javascript" src="${request.contextPath}/bigdata/v3/static/plugs/webuploader/webuploader.min.js"></script>  
	<script>
		  ...
		  picUploader.on('uploadSuccess', function (file,response) {
			...
			　　$success = $li.find('div.file-status-success');
		      ...
		      $("#"+picBusinessKey+"-relativePath").val(response.relativePath);
			...
			if($("#"+fileId+"-fullPath").val() !=""){
				$.ajax({
					...
					success:function(data) {
						if(data=="success"){
							picUploader.removeFile(picUploader.getFile(fileId),true);
							$("#"+fileId).remove();
						}else{
							showLayerTips('success','删除文件失败','t');
						}
					},
...
<#macro fileUpload  businessKey="" contextPath="" resourceUrl="" size="2" extensions="" mimeTypes="" fileNumLimit="1" handler="defaultHandler">
	<#nested>
	<div class="layer layer-addFiles layer-fileUpload height-1of1 over-y layui-layer-wrap">
			...
			<div class="file-default-box">
				...
				<div class="layer-footer">
					<a id="filePicker2" class="btn btn-white filePicker2 no-padding-top">选择文件</a>
