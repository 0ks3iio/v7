package net.zdsoft.bigdata.frame.data.redis;
...
public class BgRedisUtils {

	private static Logger logger = Logger.getLogger(BgRedisUtils.class);
	...
	public static final String KEY_PREFIX = "bigdata.";
	...
	static {
		logger.info("reids启动");
		// 加载redis配置文件
		OptionService optionService = Evn
				.getBean("optionService");
		OptionDto redisDto = optionService.getAllOptionParam("redis");
		if(redisDto !=null) {
			// 创建jedis池配置实例
			JedisPoolConfig config = new JedisPoolConfig();
			// 设置最大连接数
			config.setMaxTotal(Integer.valueOf(redisDto.getFrameParamMap().get("redis.maxActive")));
			// 获取Jedis连接的最大等待时间
			config.setMaxWaitMillis(Integer.valueOf(redisDto.getFrameParamMap().get("redis.maxWait")));
			// 最大空闲连接数
			config.setMaxIdle(Integer.valueOf(redisDto.getFrameParamMap().get("redis.maxIdle")));
			// 在获取Jedis连接时，自动检验连接是否可用
			config.setTestOnBorrow(Boolean.valueOf(redisDto.getFrameParamMap().get("redis.testOnBorrow")));
			// 在将连接放回池中前，自动检验连接是否有效
			config.setTestOnReturn(true);
			// 自动测试池中的空闲连接是否都是可用连接
			config.setTestWhileIdle(true);
			// 根据配置实例化jedis池
			if (StringUtils.isNotBlank(redisDto.getFrameParamMap().get("redis.pass"))) {
				pool = new JedisPool(config,
						redisDto.getFrameParamMap().get("redis.ip"),
						Integer.valueOf(redisDto.getFrameParamMap().get("redis.port")),
						2000, redisDto.getFrameParamMap().get("redis.pass"));
			} else {
				pool = new JedisPool(config,
						redisDto.getFrameParamMap().get("redis.ip"),
						Integer.valueOf(redisDto.getFrameParamMap().get("redis.port")));
				...
				try {
					...
					if(redisDto !=null) {
						...
						if (StringUtils.isNotBlank(redisDto.getFrameParamMap().get("redis.pass"))) {
							pool = new JedisPool(config,
									redisDto.getFrameParamMap().get("redis.ip"),
									Integer.valueOf(redisDto.getFrameParamMap().get("redis.port")), 2000,
									redisDto.getFrameParamMap().get("redis.pass"));
						} else {
							pool = new JedisPool(config,
									redisDto.getFrameParamMap().get("redis.ip"),
									Integer.valueOf(redisDto.getFrameParamMap().get("redis.port")));
	...
	private static void reSubscribe() {
		...
		for (Map.Entry<String[], JedisPubSub> pubSubEntry : pubSubMap.entrySet()) {
			BgRedisUtils.subscribe(pubSubEntry.getValue(), pubSubEntry.getKey());
		...
		for (Map.Entry<String[], JedisPubSub> entry : patternSubMap.entrySet()) {
			BgRedisUtils.pSubscribe(entry.getValue(), entry.getKey());
	...
	public static String get(String key, int time, BgRedisInterface<String> ri) {
		return get(null, key, time, ri);
	}

	public static String get(Jedis jedis, String key, int time,
			BgRedisInterface<String> ri) {
	...
	public static <T> T getObject(Jedis jedis, String key, int time,
			Class<T> clazz, BgRedisInterface<T> ri) {
	 ...
	 */
	public static <T> T getObject(Jedis jedis, String key, int time,
			int maxCount, Class<T> clazz, BgRedisInterface<T> ri) {
		String value = get(jedis, key);
		if (value == null) {
			T t = ri.queryData();
			if (maxCount > 0) {
	...
	public static <T> T getObject(Jedis jedis, String key, int time,
			int maxCount, TypeReference<T> type, BgRedisInterface<T> ri) {
		String value = get(jedis, key);
		if (value == null) {
			T t = ri.queryData();
			if (t == null) {
	...
	public static List<String> getList(String key,
			BgRedisInterface<List<String>> ri) {
	...
	public static Set<String> getSet(String key, BgRedisInterface<Set<String>> ri) {
		return getSet(null, key, ri);
	};

	public static Set<String> getSet(Jedis jedis, String key,
			BgRedisInterface<Set<String>> ri) {
	...
	public static String get(String key, BgRedisInterface<String> ri) {
		return get(key, 0, ri);
	}

	public static String get(Jedis jedis, String key, BgRedisInterface<String> ri) {
	 ...
	 *         public void run() {
	 *             BgRedisUtils.subscribe(jedisPubSub, "channel1", "channel2");
	 *         }
	 *     }).start();
	 *     or
	 *     Executors
