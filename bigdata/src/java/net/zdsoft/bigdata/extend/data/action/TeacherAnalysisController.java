 ...
 */
@Controller
@RequestMapping("/bigdata/groupAnalysis/teacher")
public class TeacherAnalysisController extends BigdataBaseAction {
	 ...
	 * @return
	 */
	@RequestMapping("/getTitleOption")
	@ResponseBody
	public Response getTitleOption(String profileCode, String tagArray) {
		List<Json> result = null;
		try {
			result = groupAnalysisBiz.queryTagResult(profileCode, tagArray, "title");
		} catch (Exception e) {
	...
	@ResponseBody
	public Response getEducation(String profileCode, String tagArray) {
		try {
			List<Json> result = groupAnalysisBiz.queryTagResult(profileCode, tagArray, "education");
			return result.size() > 0 ? Response.ok().data(result).build() : Response.error().build();
		} catch (Exception e) {
			return Response.error().message("查询出错，请检查标签是否维护正确").build();
		}
	 ...
	 * @return
	 */
	@RequestMapping("/getAgeOption")
	@ResponseBody
	public Response getAgeOption(String profileCode, String tagArray) {
		List<Json> kylinList = null;
		try {
			kylinList = groupAnalysisBiz.queryTagResult(profileCode, tagArray, "age");
		} catch (Exception e) {
			return Response.error().message("查询出错，请检查标签是否维护正确").build();
		}
		if (kylinList.size() == 0) {
