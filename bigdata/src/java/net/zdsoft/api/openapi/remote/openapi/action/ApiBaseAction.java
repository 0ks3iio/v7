...
import net.zdsoft.api.base.dto.InnerOpenDto;
import net.zdsoft.api.base.dto.OpenApiParamDto;
...
public class ApiBaseAction extends BaseAction {
    ...
    protected Logger log = Logger.getLogger(ApiBaseAction.class);

    @Autowired
    ApiBaseCommonService baseCommonService;
    @Autowired
    ...
    @Autowired
    ApiInterfaceTypeService openApiInterfaceTypeService;
    @Autowired
    ApiApplyService openApiApplyService;
    @Autowired
    ApiInterfaceCountService openApiInterfaceCountService;
    @Autowired
    ApiApplyPowerService openApiApplyPowerService;
    @Autowired
	private MetadataService metadataService;
    @Autowired
	private ApiInterfaceTypeService apiInterfaceTypeService;
    
    protected ApiDeveloper getDeveloperByRequest(HttpServletRequest request) {
		return developerService.findByTicketKey(getTicketKey(request));
	}
    
    protected Map<String, String> getAllTypeNameMap() {
		List<ApiInterfaceType> allInterfaceTypes = apiInterfaceTypeService.findAll();
		return EntityUtils.getMap(allInterfaceTypes, ApiInterfaceType::getType, ApiInterfaceType::getTypeName);
	}
    
    protected void setDeveloper(ApiDeveloper developer) {
    ...
    public JSONObject queryData(String interfaceId, String methodName, final String tableName, String resultType,
            Map<String, OpenApiParamDto> paramDtoMap, HttpServletRequest request, String nextUri, ApiDeveloper developer,
            boolean isOld, String metadataId) {
        final Map<String, String> paramMap = new HashMap<String, String>();
        for (String key : paramDtoMap.keySet()) {
            paramMap.put(key, paramDtoMap.get(key).getValue());
        }
        
        //得到数据集，把参数放进paramMap中
        ApiApply apply = openApiApplyService.findByTicketKeyAndInterfaceId(developer.getTicketKey(), interfaceId);
        if(StringUtils.isNotBlank(apply.getDataSetRuleIds())){
        	paramMap.put("dataSetRule", apply.getDataSetRuleIds());
        }
        ...
        try {
        	...
        	List<Map<String, Object>> entities = baseCommonService.getDataMapByParamMap(tableName, paramMap, columnMap, metadataId);
    ...
    protected Map<String, OpenApiParamDto> dealParam(String interfaceId, HttpServletRequest request) {
        Map<String, List<ApiParameter>> params = openApiParameterService.findParameters(interfaceId);
        Map<String, String> map = new HashMap<String, String>();
        List<ApiParameter> ps = params.get(interfaceId);
        if (CollectionUtils.isEmpty(ps)) {
            return new HashMap<String, OpenApiParamDto>();
        }
        for (ApiParameter param : ps) {
    ...
    protected Map<String, OpenApiParamDto> dealParam(String uri, JSONObject json) {
        Map<String, List<ApiParameter>> params = openApiParameterService.findParameters(uri);
        Map<String, String> map = new HashMap<String, String>();
        List<ApiParameter> ps = params.get(uri);
        if (ps == null) {
            return new HashMap<String, OpenApiParamDto>();
        }
        for (ApiParameter param : ps) {
    ...
    protected Map<String, OpenApiParamDto> dealParam(Map<String, String> map, Map<String, String> request, HttpServletRequest servletRequest) {
        ...
        if (NumberUtils.toInt(limit) > 1000 || maxLimit == null) {
            limit = "1000";
        }
        oapDto = new OpenApiParamDto();
    ...
    @SuppressWarnings("unchecked")
    protected Map<String, String> validateColumns(ApiDeveloper ticketKey, String type, String interfaceId) {
        Map<String, String> map = new HashMap<String, String>();
        String key = OPEN_API_KEY + ".VALIDATE.COLUMN." + ticketKey + "." + interfaceId;
        String json = RedisUtils.get(key);
        if (StringUtils.isBlank(json)) {
            Map<String, List<ApiEntity>> entityMap = openApiEntityService.findEntities(ticketKey,interfaceId, type);
            List<ApiEntity> openEntities = entityMap.get(type);
    ...
    protected void dealDatas(JSONObject jsonNode, Map<String, String> paramMap, List entities, ApiDeveloper developer,
...
//                                Date date = TIMESTAMP.toDate(((TIMESTAMP) o).getBytes());
    ...
    public String qd(String type, final HttpServletRequest request, String nextUri) {
        ...
        if (oi != null) {
            ...
            ApiDeveloper developer = getDeveloperByRequest(request);
            //把授权的单位放在参数列表中
            ...
            JSONObject json = queryData(oi.getId(), "queryData", oi.getTableName(), oi.getResultType(), paramDtoMap, request,
                    nextUri, developer, true, oi.getMetadataId());
...
//            doDeveloperPower(type, paramDtoMap, developer, null);
            ...
            JSONObject json = queryData(uri, "queryDataWithId", oi.getTableName(), oi.getResultType(), paramDtoMap,
                    request, nextUri, developer, (innerOpen != null ? false : true),oi.getMetadataId());
            int result = JsonUtils.getInt(json, "result");
            if (result == 0 || result == 1) {
                if (innerOpen != null) {
        ...
        if (oi != null) {
            ...
            if (StringUtils.isNotBlank(fpkName)) {
                if (StringUtils.contains(fpkName, ".")) {
                    ...
                    String uri2 = "/openapi/v3.0/" + type + "/{id}";
                    Map<String, OpenApiParamDto> paramDtoMap;
                    ApiDeveloper developer = null;
                    if (innerOpen != null) {
                        developer = innerOpen.getDeveloper();
                    ...
                    ApiInterface oi2 = openApiInterfaceService.findByUri(uri2);
                    OpenApiParamDto oapDto = new OpenApiParamDto();
                    oapDto.setParamName("id");
                    oapDto.setValue(id);
                    paramDtoMap.put("id", oapDto);
...
//                    doDeveloperPower(type, paramDtoMap, developer, null);
                    ...
                    JSONObject json = queryData(uri2, "queryDataWithId", oi2.getTableName(), oi2.getResultType(),
                            paramDtoMap, request, nextUri, developer, (innerOpen != null ? false : true), oi.getMetadataId());
	...
	private void doDeveloperPower(String interfaceId, Map<String, OpenApiParamDto> paramDtoMap, String ticketKey,String type) {
		List<ApiApplyPower> allPowers = openApiApplyPowerService.findByTicketKeyAndInterfaceIdOrType(ticketKey,interfaceId,type);
    ...
    protected List<ApiApply> getTypes(List<String> types) {
        return getTypes(types,null);
    }
	
    protected List<ApiApply> getTypes(List<String> types,Integer dataType) {
        boolean isNull = CollectionUtils.isEmpty(types);
        List<ApiApply> list = new ArrayList<>();
    ...
    protected List<String[]> getInterfaceTypes (Integer dataType){
    	...
    	Integer[] types = {ApiInterfaceType.INTERFACE_TYPE,ApiInterfaceType.PUBLIC_TYPE};
		List<ApiInterfaceType>  interfaceTypes = openApiInterfaceTypeService.findByClassifyIn(types);
        Map<String, String> resultTypeMap = EntityUtils.getMap(interfaceTypes, ApiInterfaceType::getType, ApiInterfaceType::getTypeName);
