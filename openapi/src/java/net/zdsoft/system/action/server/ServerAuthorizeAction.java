...
public class ServerAuthorizeAction extends BaseAction {
    ...
    @Autowired
    private ServerAuthorizeRemoteService serverAuthorizeRemoteService;
    @Autowired
    ...
    public String unitAuthorizeView(String region, String unitName, HttpServletRequest request, String regionName,
        ...
        if (CollectionUtils.isNotEmpty(unitList)) {
            Set<String> unitIdSet = EntityUtils.getSet(unitList, UnitDto::getUnitId);
            Map<String, List<Server>> unitIdAndAuthorizeServerMap = serverAuthorizeRemoteService
    ...
    public String modifyUnitAuthorizeView(String unitId, int unitClass) {
        // 可授权应用列表,不考虑学段条件
        List<Server> serverList = serverRemoteService.findServerList(
        ...
        if (CollectionUtils.isNotEmpty(serverList)) {
            // 单位已订阅应用id
            List<Integer> authorizeServerIds = serverAuthorizeRemoteService.listServerIds(unitId);
    ...
    public String saveUnitAuthorize(String[] serverIds, @PathVariable String unitId) {
        ...
        serverAuthorizeRemoteService.updateUnitAuthorize(unitId, serverAuthorizeList);
    ...
    public String serverAuthorizeView(Integer serverId, String unitType, Integer authorizeStatus, String region,
        ...
        // 应用列表
        List<Server> serverList = serverRemoteService.findByOrderTypeAndStatus(new Integer[] {
    ...
    public String allAuthorize(@PathVariable Integer serverId, String unitType, int authorizeStatus, String region) {
        ...
        if (authorizeStatus == YesNoEnum.NO.getValue()) {// 取消已授权单位
        	serverAuthorizeRemoteService.removeByServerIdAndUnitCondition(serverId, region, unitClass);
        }
            ...
            if (CollectionUtils.isNotEmpty(unitList)) {
                ...
                serverAuthorizeRemoteService.saveServerAuthorizes(serverAuthorizeList);
            }
        }
    ...
    private void authorize(String[] unitIds, Integer serverId, int authorizeStatus) {
        if (authorizeStatus == YesNoEnum.YES.getValue()) {// 授权
        	...
        	serverAuthorizeRemoteService.removeServerAuthorizes(serverId, unitIds);
        }
    }
}
