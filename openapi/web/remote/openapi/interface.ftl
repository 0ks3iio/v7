<script type="text/javascript" src="${resourceUrl}/remote/openapi/js/myscript.js"></script>
    ...
    <div class="app-wrap">
    	<p class="dt">
            ...
            <span class="path">${openApi.uri!}</span>
        ...
        <div class="dd">
            ...
            <table class="table-list public-table">
                ...
                <tr class="tr-${entity.id!}">
                    ...
                    <a href="javascript:void(0);" onclick="javascript:addParam('${openApi.uri!}', '${entity.entityName!}', '${entity.type!}', '${entity.id!}');">参数</a>
                    </td>
                    </#if>
            ...
            <table class="table-list public-table">
                ...
                <tr id="tr-${param.id!}">
                	<#if adminLogin?default("0") == "1">
                    <td>
                    <a href="javascript:void(0);" onclick="javascript:deleteParam('${param.uri!}', '${param.paramName!}', '${param.id!}');">取消</a>
                    </td>
                    </#if>
                    <td>${param.paramName}</td>
                    <#if param.mcodeId?default("") != "" && mcodeMap[param.mcodeId!]?exists>
                    	<td><select onchange="javascript:inputChanged('${openApi.id!}', '/remote${openApi.uri!}');" class="input-sel" style="width:120px;" id="${param.paramName}" name="${param.paramName}" txttype="${openApi.id!}">
                    ...
                    <#else>
                    <td><input onblur="javascript:inputChanged('${openApi.id!}', '/remote${openApi.uri!}');" mandatory="${param.mandatory?default(0)}" class="input-txt params" style="width:112px;" txttype="${openApi.id!}" id="${param.paramName}" name="${param.paramName}"></input></td>
                    </#if>
            ...
            <div class="ui-tab">
                ...
                <div class="ui-tab-wrap">
                    <div class="ui-tab-module">
                        <code class="rainbow">                                    
    Request.Get("${request.contextPath!"http://api.wanpeng.com"}<span class="${openApi.id!}-uri-span">/remote${openApi.uri!}</span><span class="ticketKey-span">?ticketKey=${ticketKey!}</span><span class="${openApi.id!}-param-span"></span>").execute().returnContent().asString();</code>
                        <p class="mt-10"><a href="#" class="abtn-blue get-code" onclick="javascript:queryJson('${openApi.id!}', '${openApi.uri!}');"><span class="query-button-span">获取</span></a></p>
                    </div>
                    <div class="ui-tab-module" style="display:none;">
                     <code class="rainbow">
	$ch = curl_init('${request.contextPath!"http://api.wanpeng.com"}<span class="${openApi.id!}-uri-span">/remote${openApi.uri!}</span><span class="ticketKey-span">?ticketKey=${ticketKey!}</span><span class="${openApi.id!}-param-span"></span>');
	...
	<br>$response = json_decode(curl_exec($ch));
                    ...
                    <div class="ui-tab-module" style="display:none;">
                    <code class="rainbow">
		curl -X GET ${request.contextPath!"http://api.wanpeng.com"}<span class="${openApi.id!}-uri-span">/remote${openApi.uri!}</span><span class="ticketKey-span">?ticketKey=${ticketKey!}</span><span class="${openApi.id!}-param-span"></span>
					</code>
						<p class="mt-10"><a href="#" class="abtn-blue get-code" onclick="javascript:queryJson('${openApi.id!}', '${openApi.uri!}');"><span class="query-button-span">获取</span></a></p>
					</div>
                </div>
...
function queryJson(id, uri){
	...
	console.info('${request.contextPath}/remote' + uri);
	var aj = $.ajax({  
	    ...
	    beforeSend:function(XMLHttpRequest){
	    	...
	    	XMLHttpRequest.setRequestHeader("ticketKey",ticketKey);
	    },  
	    success:function(data) {  
...
function hideEntity(entityName, type, id){
	...
	objs.id = id;
	$.ajax({  
	    url:'${request.contextPath}/remote/openapi/entity/hide',
	    data: $.param(objs),  
	    type:'post',  
	    cache:false,  
	    dataType:'json',
...
function deleteParam(uri, paramName, id){
	...
	objs.id = id;
	$.ajax({  
	    url:'${request.contextPath}/remote/openapi/parameter/' + id,
	    data: $.param(objs),  
	    type:'delete',  
	    cache:false,  
	    dataType:'json',
...
function addParam(uri, entityName, type, id){
	...
	objs.uri = uri;
	$.ajax({  
	    url:'${request.contextPath}/remote/openapi/parameter',
	    data: $.param(objs),  
	    type:'post',  
	    cache:false,  
	    dataType:'json',
...
function changeMcode(id){
	...
	$.ajax({  
	    url:'${request.contextPath}/remote/openapi/entity/mcodeId',
	    data: $.param(objs),  
	    type:'post',  
	    cache:false,  
	    dataType:'json',
