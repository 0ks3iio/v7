...
strVarCity += '                  <tr>';
strVarCity += '                  <tr>';
strVarCity += '                    <td class="aui_icon">';
strVarCity += '                     <div class="aui_iconBg"></div></td>';
strVarCity += '                       <td class="aui_main">';
strVarCity += '                        <div class="aui_content">';
strVarCity += '                          <div id="">';
strVarCity += '                            <div class="data-result"><em>最多选择 <strong>2000</strong> 项</em>：</div>';
strVarCity += '                            <div class="data-error">最多只能选择 3 项</div>';
strVarCity += '                            <div class="data-search" id="searchRun"><input type="text" class="run" name="searcharea"/><div class="searchList run"></div></div>';
...
function appendCity(thiscon, Cityxz) {
    ...
    $('.aui_outer .aui_header').on("mousedown", function (e) {
        ...
        $('.aui_outer').css({
                left: $(this).offset().left - $(window).scrollLeft()-1,
                top: $(this).offset().top - $(window).scrollTop()-1,
                margin: 'auto'
            });
        $(this).on("mousemove", function (e) {
           $('.aui_outer').css({
                left: e.pageX  - x - $(window).scrollLeft()-1,
                top: e.pageY  - y - $(window).scrollTop()-1
            }); 
        }).on("mouseup", function () {
...
function selectProvince(type, con, isremove) {
    ...
    if (type == "all") {
        ...
        for (var i in datahotcityxz) {
            strVarCity += '<li><a href="javascript:;" data-code="' + datahotcityxz[i] + '" data-name="' + dataarrary[datahotcityxz[i]][0] + '" class="d-item"  onclick="selectProvince(\'sub\',this,\'\')">' + dataarrary[datahotcityxz[i]][0] + '</a><label>0</label></li>';
        }
        strVarCity += '      </ul>';
        strVarCity += '    </div>';
        ...
        for (var i in dataCityxz) {
            strVarCity += '<li><a href="javascript:;" data-code="' + dataCityxz[i] + '" data-name="' + dataarrary[dataCityxz[i]][0] + '" class="d-item"  onclick="selectProvince(\'sub\',this,\'\')">' + dataarrary[dataCityxz[i]][0] + '</a><label>0</label></li>';
        }
        strVarCity += ' </ul>';
        strVarCity += '</div>';
            ...
            if ($('a[data-code=' + $(this).data("code") + ']').length > 0) {
                ...
                if ($('a[data-code=' + $(this).data("code") + ']').attr("class").indexOf('data-all') > 0) {
                    ...
                    if ($('.data-list-provinces a[data-code=' + $(this).data("code").toString().substring(0, 3) + ']').length > 0) {
                        var numlabel = $('.data-list-provinces a[data-code=' + $(this).data("code").toString().substring(0, 3) + ']').next('label').text();
                        $('.data-list-provinces a[data-code=' + $(this).data("code").toString().substring(0, 3) + ']').next('label').text(parseInt(numlabel) + 1).show();
                    }
                }
            } else {
                var numlabel = $('.data-list-provinces a[data-code=' + $(this).data("code").toString().substring(0, 3) + ']').next('label').text();
    ...
    //显示下一级
        ...
        if (typeof (relations[$(con).data("code")]) != "undefined") {
            ...
            if (datatype == "danxuan") {
                ...
                strVarCity += '<li class="" style="width:100%"><a href="javascript:;" class="d-item data-all"  data-code="' + $(con).data("code") + '"  data-name="' + $(con).data("name") + '"  onclick="selectitem_area(this)">' + $(con).data("name") + '</a><label>0</label></li>';
            }
            for (var i in relations[$(con).data("code")]) {
                strVarCity += '<li><a href="javascript:;" class="d-item" data-code="' + relations[$(con).data("code")][i] + '"  data-name="' + dataarrary[relations[$(con).data("code")][i]][0] + '" onclick="selectProvince(\'sub\',this,\'\')">' + dataarrary[relations[$(con).data("code")][i]][0] + '</a><label>0</label></li>';
            }
            strVarCity += '</ul>';
            $('.data-container-city').html(strVarCity);
        } else {
            if (datatype == "duoxuan") {
                ...
                if ($(con).attr("class").indexOf('d-item-active') > 0) {
                    ...
                    if ($('.data-list-provinces a[data-code=' + $(con).data("code").toString().substring(0, 3) + ']').length > 0) {
                        var numlabel = $('.data-list-provinces a[data-code=' + $(con).data("code").toString().substring(0, 3) + ']').next('label').text();
                        if (parseInt(numlabel) == 1) {
                            $('.data-list-provinces a[data-code=' + $(con).data("code").toString().substring(0, 3) + ']').next('label').text(0).hide();
                        } else {
                            $('.data-list-provinces a[data-code=' + $(con).data("code").toString().substring(0, 3) + ']').next('label').text(parseInt(numlabel) - 1);
                        }
                    }
                    return false;
                } else {
        ...
        $('.data-result span').each(function () {
            $('.data-list-provinces a[data-code=' + $(this).data("code").toString().substring(0, 3) + ']').next('label').text(0).hide();
        });
        $('.data-result span').each(function () {
            if ($('a[data-code=' + $(this).data("code") + ']').length > 0) {
                $('a[data-code=' + $(this).data("code") + ']').addClass('d-item-active');
...
function removespan_area(spanthis) {
    ...
    if ($('.data-list-provinces a[data-code=' + $(spanthis).data("code").toString().substring(0, 3) + ']').length > 0) {
        var numlabel = $('.data-list-provinces a[data-code=' + $(spanthis).data("code").toString().substring(0, 3) + ']').next('label').text();
        if (parseInt(numlabel) == 1) {
            $('.data-list-provinces a[data-code=' + $(spanthis).data("code").toString().substring(0, 3) + ']').next('label').text(0).hide();
        } else {
            $('.data-list-provinces a[data-code=' + $(spanthis).data("code").toString().substring(0, 3) + ']').next('label').text(parseInt(numlabel) - 1);
        }
    }
    $(spanthis).remove();
}
