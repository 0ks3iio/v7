function initLoginSygh(urlPath){
    var mobilePhone = $.cookies.get(Constants.LOGIN_SYGH_COOKIE_MOBILEPHONE_KEY);
    var password = $.cookies.get(Constants.LOGIN_SYGH_COOKIE_PASSWORD_KEY);
    if (password == "\"\"") {
        password = '';
    }
    if (mobilePhone) {
...
var isSubmit=false;
function doLogin(urlPath,autoLogin) {
	if(isSubmit){
		return;
	}
	var mobilePhone = $("#mobilePhone").val();
	if(mobilePhone==""){
		$(".login-tips").text("请输入手机号");
		return;
	}else{
		var mobilePhoneReg = /^1(3|4|5|6|7|8|9)\d{9}$/;
		if(!mobilePhoneReg.test(mobilePhone)){
			$(".login-tips").text("请输入正确的手机号码");
			return;
		}
	}
    ...
    cookieExpires.setTime(cookieExpires.getTime() + (1000 * 60 * 60 * 24 * Constants.LOGIN_SYGH_COOKIE_LIFE));
    var cookieSaveType = 1;
    if ($("#cookieSaveType").is(':checked')) {
        cookieSaveType = 2;
    }
    if (cookieSaveType == 2) {
        $.cookies.set(Constants.LOGIN_SYGH_COOKIE_MOBILEPHONE_KEY, mobilePhone, {
            expiresAt: cookieExpires
        });
        $.cookies.set(Constants.LOGIN_SYGH_COOKIE_PASSWORD_KEY, password, {
            expiresAt: cookieExpires
        });
    } else if (cookieSaveType == 1) {
        $.cookies.del(Constants.LOGIN_SYGH_COOKIE_MOBILEPHONE_KEY);
        $.cookies.del(Constants.LOGIN_SYGH_COOKIE_PASSWORD_KEY);
    }
    
	isSubmit = true;
	
	var options = {  
		url:urlPath+'/homepage/register/syghLogin/login',
		data:{"mobilePhone":mobilePhone,"password":password,"imgCode":imgCode,"autoLogin":autoLogin},
		clearForm : false,
   		resetForm : false,
		dataType:'json',
		type:'post',
		success:function(data){
			if(data.success){
				window.location.href=urlPath+'/homepage/register/syghLogin/loginForPassport?username='+data.msg;
			}else{
				isSubmit=false;
				flashCodeImg(urlPath);
...
function sendcode(num,obj,count,n,urlPath,type){
	...
	$(obj).click(function(e){
		...
		if($(this).hasClass('get-code-able')){
			$.ajax({
	            url:urlPath+'/homepage/register/syghGetMsgCode',
