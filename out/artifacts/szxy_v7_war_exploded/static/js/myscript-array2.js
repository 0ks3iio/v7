...
$(function(){
	...
	$('.content .syllabus-table td .item').on('click','.replace a',function(e){
		...
		param += "&leftSchedules[0].teacherId="+left_teacherId;
		...
		if(period) param += "&period=" + $(this).parents("td").attr("data-time");
					...
					if(!$(this).hasClass('item-sel')){
						...
						if(sel==0){
							...
							if($(this).find(".s-d-week").length > 0){
								var subjIds = user.split(",");
//								console.log("单双周禁排   "+subjIds);
								if(!course_limit) subjIds=[];
								for(i in subjIds){
								...
								var notime = subjectNoTime[user];
								if(!course_limit) notime = false;
								insMainNoTime(notime);
							}
							...
							if($(this).find(".s-d-week").length > 0){
								...
								var subjectId_now = $(this).find("[name='subjectId']:eq(1)").val();
...
//							return ;
							var arrayId = indexId;
							var url = contextPath + "/newgkelective/scheduleModify/"+arrayId+"/swapCourse";
							var obj = this;
							$.ajax({
								url: url,
								data:param,
...
//					autoTips('已预约课程不能移动');
	...
	$('html').on('click','body > .item',function(){
		...
		$('.syllabus-table td .item-subject-no').each(function(){
			$(this).removeClass('item-lock').removeClass("item-subject-no");
	...
	//滑过
	$('.content .syllabus-table td .item,.syllabus-from li').not('.item-lock').mouseover(function(){
		//获取教师课表
		var teacherId1 = $(this).find("[name='teacherId']:eq(0)").val();
		var teacherId2 = $(this).find("[name='teacherId']:eq(1)").val();
...
//					console.log("timestr = "+timestr+(timestr == theTime));
			...
			var subjectId = $("#"+tableId+" tbody td[data-time='"+theTime+"']").find(".item").attr("data-user");
	...
	// 填写老师课表
	function getTeSchedule(teacherIds){
		...
		var arrayId = indexId;
		var url = contextPath + "/newgkelective/"+arrayId+"/teacherSchedule?";
		//console.log("url: "+url);
		$.ajax({
			url: url,
	...
	function insTeaSchedule(teacherIds, scheduleObj){
		...
		if(teacherIds.length > 1){
			...
			spantitle = '<span class="tab active" data-tid="'+teacherIds[0]+'">\
			...
			spantitle = '<span class="mr-5 ml-5">'+tname+'</span>老师课程表';
		}
		$("#teacherTable1").prev("p").html(spantitle);
...
//						console.log(notime);
					...
					var notime = subjectNoTime[cln_user];
					if(course_limit) insMainNoTime(notime);
				}
					...
					if(!$(this).hasClass('item-lock')){
						...
						if(data_sel==1){
							if(!$(this).hasClass('item-can')){
								...
								var arrayId = indexId;
								var url = contextPath + "/newgkelective/scheduleModify/"+arrayId+"/delOrAddLecture";
								var obj = this;
								$.ajax({
									url: url,
									data:param,
...
//	var filter_cnt2 = $('.sidebar-left .sidebar-filter .list:eq(1)').html();
	function getFilterCnt(index){
		return index==0?filter_cnt1:filter_cnt2;
	}
	var f = false;
...
function insScheduleOnly(id,schedules){
	...
	for(i in schedules){
		...
		var oldSubjId = $("#"+id+" tbody [data-time='"+timestr+"'] .item").attr("data-user");
		if(oldSubjId && oldSubjId!="0"){  // 单双周
			...
			$("#"+id+" tbody [data-time='"+timestr+"'] .item").attr("data-user",oldSubjId+","+subjectId);
		}else{
			//console.log("i = "+i+"  x = "+x);
			$("#"+id+" tbody [data-time='"+timestr+"'] .item").attr("data-user",subjectId);
		}
