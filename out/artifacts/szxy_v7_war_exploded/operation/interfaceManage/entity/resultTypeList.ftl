...
function modifyType(){
  ...
  $("#tabList").load("${request.contextPath}/system/interface/entity/index?type="+type);
}   
...
function deleteType(){
	...
	function(){
		$.ajax({
            url:"${request.contextPath}/system/interface/entity/delEntityByType?type="+type,
            data:{},
            dataType:'json',
            ...
            success:function (data) {
                if(data.success){
                    showSuccessMsgWithCall(data.msg,showTab(2));
                }else{
