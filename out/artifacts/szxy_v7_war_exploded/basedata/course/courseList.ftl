	...
	<table-hover>
		...
		<#list courseList as item>
			<tr>
				...
				<td><span>${item.subjectName!}</span><#if item.bgColor?default('')!=''><span class="fill" id="title" style="background-color: ${item.bgColor?default('')}"></span></#if></td>
				<td><span>${item.shortName!}</span></td>
				<td><span>${item.courseTypeName!}</span></td>
				<td><span>${item.sectionName!}</span></td>
				<td><span>${item.initCredit!}</span></td>
				<td><span>${item.fullMark!}</span></td>
				<td><span>${item.initPassMark!}</span></td>				
...
$(function(){
	...
	//编辑课程
	$('.js-editCourse').unbind("click").bind('click',function(e){
...
function deleteByCourseIds(idArray){
	...
	$.ajax({
		   ...
		   success: function(msg){
			 if(msg.success){
			 	layer.msg(msg.msg, {offset: 't',time: 3000});
				refreshPage();
			 }else{
...
function editCourse(courseId){
	...
	indexDiv = layerDivUrl(url,{title: title,width:650,height:500});
}
</script>