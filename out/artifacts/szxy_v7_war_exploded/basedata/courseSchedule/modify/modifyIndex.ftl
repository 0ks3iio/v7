...
function manageClaorTea(id,type, useMaster){ // classId | teacherId
	...
	var url = '${request.contextPath}/'+url_sub_sys+'/scheduleModify/${indexId!}/queryClassOrTeacherTable/page?';
...
function updateTeaConflict(){
	<#if sub_sys! == "BASE">return;</#if>
	var url = '${request.contextPath}/'+url_sub_sys+'/scheduleModify/${indexId!}/getConflictTeachers';
	$.ajax({
		url: url,
		type: "POST",
		dataType: "JSON",
		success:function(data){
			...
			if(data.success){
				//更新 时间
				var conflictTids = data["conflictTids"];

				$("#tidList li").each(function(i,obj){
					var tid = $(obj).attr("data-class");
					if(tid && conflictTids.indexOf(tid)>-1){
						$(obj).find(".conflict").text("*");
					}else{
						$(obj).find(".conflict").text("");
					}
				});
			}else{
				console.error("更新未解决冲突教师失败: "+data.msg);
			}
		},
		error:function(){}
	});
...
function updateMovePerios(){
	...
	<#if sub_sys! == "BASE">return;</#if>
	var url = '${request.contextPath}/'+url_sub_sys+'/scheduleModify/${indexId!}/updateMovePeriods';
	$.ajax({
		url: url,
		type: "POST",
...
function coverAll(){
	...
	var url = '${request.contextPath}/'+url_sub_sys+'/scheduleModify/${indexId!}/coverAllWeek';
...
function getSurplusInf(){
	...
	var url = '${request.contextPath}/'+url_sub_sys+'/scheduleModify/${indexId!}/classSurplusHour';
