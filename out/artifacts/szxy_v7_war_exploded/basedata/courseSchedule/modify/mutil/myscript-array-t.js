...
$(function(){
	...
	$('.content .syllabus-table td .item').on('click','.replace a',function(e){
		...
		var arrayId = indexId;
		var url = contextPath + "/"+url_sub_sys+"/scheduleModify/mutil/"+arrayId+"/swapWeekType?"+arrayIdParams;
		$.ajax({
			url: url,
			data:param,
			type:"POST",
				...
				if(!$(this).hasClass('item-lock')){
					...
					if(!$(this).hasClass('item-sel')){
						...
						if(sel==0){
							...
							var classId1 = $(this).find("[name='classId']").val();
							var subjectId2 = $(this).find("[name='subjectId']:eq(1)").val();
							var classId2 = $(this).find("[name='classId']:eq(1)").val();
							if(course_limit && subjectId1){
								var notime = subjectNoTime[classId1+"_"+subjectId1];
								insMainNoTime(notime);
							}
							// 组合固定 禁排
							if(sub_sys != "BASE"){
							...
							if($(this).find(".s-d-week").length > 0){
								if(course_limit && subjectId2){
									var notime = subjectNoTime[classId2+"_"+subjectId2];
									insMainNoTime(notime);
								}
...
//							if(!f || !g) return;
							...
							if(to_user != 0){
								...
								clsIds.push(classId1);
								if(subjectId2){
									subIds.push(subjectId2);
									clsIds.push(classId2);
								}
								for(i in subIds){
									var subId = subIds[i];
									var clsId = clsIds[i];
									var notime = subjectNoTime[clsId+"_"+subId];
									if(course_limit && notime.search(old_time) > -1){
							...
							for(i in tids){
								...
								if(teacher_limit && notime.search(old_time) > -1){
...
//							return ;
							var arrayId = indexId;
							var url = contextPath + "/"+url_sub_sys+"/scheduleModify/mutil/"+arrayId+"/swapTeacherCourse?"+arrayIdParams;
							var obj = this;
							$.ajax({
								url: url,
								data:param,
...
//					autoTips('已预约课程不能移动');
					if($(this).hasClass("movePeriod")){
						if(url_sub_sys == 'basedata'){
							autoTips('走班时间或其他年级课程，此时间禁排');
						}else{
	...
	$('.content .syllabus-table td .item,.syllabus-from li').not('.item-lock').mouseenter(function(){
		...
		setTimeout(function(){
			if(timestr == nowTdf){
				...
				var classIds = [classId1, classId2];
				getTeSchedule(classIds);
...
//					console.log("timestr = "+timestr+(timestr == theTime));
	...
	function getTeSchedule(teacherIds,fix){
		...
		var arrayId = objArrayIdMap[teacherIds[0]];
		var url = contextPath + "/"+url_sub_sys+"/scheduleModify/mutil/"+arrayId+"/classSchedule?"+arrayIdParams;
		var searchWeek = $(".ui-select-value").val();
		console.log("classIds: "+teacherIds);
		//return;
		$.ajax({
			url: url,
			data: {"classIds" : teacherIds,"searchAcadyear":searchAcadyear,"searchSemester":searchSemester,"weekIndex":searchWeek},
			success:function(data){
				insTeaSchedule(teacherIds,data);
				
				if(fix) fixTeacherTable();
				if(fix ==2) forbidTime(teacherIds,data);
			},
			dataType:"JSON",
			type:"POST"
		});
	...
	function forbidTime(teacherIds,scheduleObj){
		for(x in teacherIds){
			var tid = teacherIds[x];
			var schedules = scheduleObj[tid];
			...
			for(i in schedules){
				...
				$("#allTable tbody [data-time='"+timestr+"'] .item").removeClass("item-can").addClass("item-conflict");
			}
		}
	}
	...
	function insTeaSchedule(teacherIds, scheduleObj){
		...
		if(teacherIds.length > 1){
			...
			var tname1 = classNameMap[teacherIds[0]];
			var tname2 = classNameMap[teacherIds[1]];
			...
			var tname = classNameMap[teacherIds[0]];
	...
	// 点击后显示下方课表
	function fixTeacherTable(obj,pt){
		if(obj){
			var classId1 = $(obj).find("[name='classId']:eq(0)").val();
			var classId2 = $(obj).find("[name='classId']:eq(1)").val();
			var classIds = [classId1, classId2];
			if(!pt) pt = true;
			getTeSchedule(classIds,pt);
			console.log("--获取教师信息----");
		}else{
...
//		var $str2='<span class="course" style="height:'+span_h+';line-height:'+span_h+'">'+into_course+'</span><span class="name" style="height:'+span_h+';line-height:'+span_h+'">'+into_name+'</span><span class="room" style="height:'+span_h+';line-height:'+span_h+'">'+into_room+'</span>';
		...
		if(from_user == into_user || $(obj).hasClass('empty')){
			...
			if(all_len != 0){
				...
				$('.int-from').stop(true,true).animate({top:into_top,left:into_left},2000,function(){
		...
		if(data_lock == 0){
			...
			if(!$(this).hasClass('over')){
				...
				if(s_d_week){
					...
					var clsIds = [classId,classId2];
				    //console.log("单双周禁排   "+subjIds);
					for(i in subjIds){
						var notime = subjectNoTime[clsIds[i]+"_"+subjIds[i]];
//						console.log(notime);
					...
					var subjectId = $(this).find("[name='subjectId']").val();
					var classId = $(this).find("[name='classId']").val();
					var notime = subjectNoTime[classId+"_"+subjectId];
					if(course_limit) insMainNoTime(notime);
				}
				
				
				// 
				fixTeacherTable(this,2);
				$('body').attr('data-sel',1).children('.cln-item').remove();
					...
					if(!$(this).hasClass('item-lock')){
						...
						if(data_sel==1){
							if(!$(this).hasClass('item-can')){
								...
								isSaving = true;
								var outer_weekType1 = $(this).find("[name='weekType']").val();
								if(outer_weekType1 && outer_weekType1 != '3'){
									isSaving = false;
									autoTips("教师侧不能删除单双周课程");
									return;
								}
...
//								return ;
								var arrayId = indexId;
								var url = contextPath + "/"+url_sub_sys+"/scheduleModify/mutil/"+arrayId+"/delOrAddLecture?"+arrayIdParams;
								var obj = this;
								$.ajax({
									url: url,
									data:param,
...
//											autoTips("操作成功");
			...
			var outer_weekType1 = $(".int-from").find("[name='weekType']").val();
			if(outer_weekType1 && outer_weekType1 != '3'){
				isSaving = false;
				autoTips("教师侧不能删除单双周课程");
				return;
			...
			console.log("test");
			var len = $(".syllabus-from li[data-user='"+outer_classId1+"_"+outer_subjectId1+"']").length;
			if(len <=0){
				isSaving = false;
				autoTips("不能删除教学班课程");
				return;
