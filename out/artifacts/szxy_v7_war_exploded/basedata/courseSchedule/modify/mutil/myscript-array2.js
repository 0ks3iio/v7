...
$(function(){
	...
	$('.content .syllabus-table td .item').on('click','.replace a',function(e){
		...
		var arrayId = indexId;
		var url = contextPath + "/newgkelective/scheduleModify/mutil/"+arrayId+"/swapWeekType?"+arrayIdParams;
		$.ajax({
			url: url,
			data:param,
			type:"POST",
					...
					if(!$(this).hasClass('item-sel')){
						...
						if(sel==0){
							...
							if(course_limit && subjectId1){
								var notime = subjectNoTime[objId+"_"+subjectId1];
								insMainNoTime(notime);
							}
							
							if($(this).find(".s-d-week").length > 0){
								if(course_limit && subjectId2){
									var notime = subjectNoTime[objId+"_"+subjectId2];
									insMainNoTime(notime);
								}
...
//							if(!f || !g) return;
							...
							if(to_user != 0){
								...
								for(i in subIds){
									var subId = subIds[i];
									var notime = subjectNoTime[objId+"_"+subId];
...
//							return ;
							var arrayId = indexId;
							var url = contextPath + "/"+url_sub_sys+"/scheduleModify/mutil/"+arrayId+"/swapCourse?"+arrayIdParams;
							var obj = this;
							$.ajax({
								url: url,
								data:param,
...
//					console.log("timestr = "+timestr+(timestr == theTime));
	...
	function checkConflict(obj,time,weekType,subType,tid,subId){
		...
		notime = subjectNoTime[objId+"_"+subId];
		...
		if(data_lock == 0){
			...
			if(!$(this).hasClass('over')){
				...
				if(s_d_week){
					...
					for(i in subjIds){
						var notime = subjectNoTime[objId+"_"+subjIds[i]];
//						console.log(notime);
					...
					var subjectId = $(this).find("[name='subjectId']").val();
					var notime = subjectNoTime[objId+"_"+subjectId];
					if(course_limit) insMainNoTime(notime);
				}
