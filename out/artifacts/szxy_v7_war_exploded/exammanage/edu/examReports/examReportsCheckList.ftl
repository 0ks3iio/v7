			...
			<#--<div class="col-xs-6">
				<table class="table table-bordered">
					<tbody>
						<tr>
							<td>
								<p>考号：${item.examNumber!}</p>
								<p>姓名：${item.studentName!}</p>
								<p>所在学校：${item.schName!}</p>
							</td>
							<td width="71"><img src="${request.contextPath}${item.studentFilePath!}" width="71" height="99"></td>
							<td width="120"><img width="120" height="174" src="${request.contextPath}${item.studentFilePath!}"></td>
						</tr>
					</tbody>
				</table>
			</div> -->
...
<script>
	...
	function goSearch(regionId,optionId,placeId){
		...
		var url ='${request.contextPath}/exammanage/edu/examReports/detailList/page?examId='+examId+"&regionId="+regionId+"&optionId="+optionId+"&placeId="+placeId+"&regionPick="+regionPick+"&optionPick="+optionPick+"&type=2"+"&numPick="+numPick;
		$("#showTabDiv").load(url);
	}
