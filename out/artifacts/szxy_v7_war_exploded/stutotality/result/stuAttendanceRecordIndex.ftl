...
<div class="box box-default" >
    <div class="box-body clearfix">
        ...
        <div class="evaluate-item" style="padding-top: 0;padding-bottom:10px;">
...
<script>
    ...
    function getAttendanceList(gradeId,classId) {
        var url = '${request.contextPath}/stutotality/result/getStuAttendanceRecordList?gradeId='+gradeId+"&classId="+classId;
        $("#divList").load(url)
    }

    function showalledit(){
        var classId = $("#outter2").find("a.selected").attr("data-id")
        var gradeId = $("#outter1").find("a.selected").attr("data-id")
        if(classId == '' || classId == undefined){
            layerTipMsg(false,"请选择一个班级!","");
            return;
        }

        $("#divImport").load("${request.contextPath}/stutotality/stuAttendanceRecord/import/main?classId="+classId+"&gradeId="+gradeId);
    }
</script>