<div class="layer layer-addTerm layer-change" style="display:block;" id="myDiv">
    <form id="subForm">
        ...
        <input type="hidden" name="state" id="planState" <#if famDearPlan??>value=${famDearPlan.state!}</#if> ">
            ...
            <div class="filter clearfix">
                <div class="filter clearfix">
                    <div class="filter-item" style="width:270px;">
                        ...
                        <select class="form-control" id="acadyear" name="year" <#if type?default("2")=="2">disabled="disabled"</#if>>
                            <#if acadyearList?exists && (acadyearList?size>0)>
                                <#list acadyearList as item>
                                    <#if (nowYear?eval <=item?eval)><option value="${item!}" <#if famDearPlan.year?default('a')==item?default('b')>selected</#if>>${item!}</option></#if>
                                </#list>
                            <#else>
                    ...
                    <div class="filter-item">
                        ...
                        <div class="filter-content" style="width:470px;">
                            <input type="text" class="form-control" id="title1" name="title" maxlength="100" style="width:100%;" value="${famDearPlan.title!}" <#if type?default("2")=="2">disabled="disabled"</#if> />
                        </div>
                    </div>
                    <div class="filter-item">
                        <label for="" class="filter-name">指导思想：</label>
                        <div class="filter-content" style="width:470px;">
                            <textarea class="form-control" maxlength="500" id="guideThought" name="guideThought"   style="width:100%;" <#if type?default("2")=="2">disabled="disabled"</#if>>${famDearPlan.guideThought!}</textarea>
                        </div>
                    </div>
                    <div class="filter-item">
                        <label for="" class="filter-name">工作目标：</label>
                        <div class="filter-content" style="width:470px;">
                            <textarea type="text" class="form-control" maxlength="500" id="workObjective" name="workObjective"   style="width:100%;" <#if type?default("2")=="2">disabled="disabled"</#if> >${famDearPlan.workObjective!}</textarea>
                        </div>
                    </div>
                    <div class="filter-item">
                        <label for="" class="filter-name">主要内容：</label>
                        <div class="filter-content" style="width:470px;">
                            <textarea type="text" class="form-control" maxlength="500" id="mainTask" name="mainTask"    style="width:100%;" <#if type?default("2")=="2">disabled="disabled"</#if>>${famDearPlan.mainTask!}</textarea>
                        </div>
                    </div>
                    <div class="filter-item">
                        <label for="" class="filter-name">年度活动安排：</label>
                        <div class="filter-content" style="width:470px;">
                            <textarea type="text" class="form-control"  maxlength="1000" id="activityArrange" name="activityArrange"   style="width:100%;" <#if type?default("2")=="2">disabled="disabled"</#if> >${famDearPlan.activityArrange!}</textarea>
                        </div>
                    </div>
                <#--
                <div class="filter-item">
...
<script>
    ...
    function showPlanList(){
        ...
        var url =  '${request.contextPath}/familydear/plan/famDearPlanList?year='+year+"&title="+title+"&_pageIndex="+currentPageIndex+"&_pageSize="+currentPageSize;
        $("#showList1").load(url);
    }
