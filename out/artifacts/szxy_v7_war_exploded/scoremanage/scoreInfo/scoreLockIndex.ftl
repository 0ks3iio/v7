...
<div class="box box-default">
    <div class="box-header header_filter">
        <div class="filter filter-f16">
			...
			<div class="filter-item">
				...
				<div class="filter-content">
					<select class="form-control" style="width:105px;" id="semesterSearch" onChange="changeExam()">
	        ...
	        <div class="filter-item">
				...
				<div class="filter-content">
					<select class="form-control" style="width:120px;" id="gradeCode" onChange="changeSubject()">
						<option value="">---请选择---</option>
					</select>
				</div>
			</div>	
			<div class="filter-item" id="findDiv">
				...
				<div class="filter-content" id="subjectIdDiv">
					<select class="form-control" id="subjectInfoId" onChange="searchList()">
						<option value="">---请选择---</option>
					</select>
				</div>
			</div>
	...
	<div class="box-body" id="lockList">	
	</div>
</div>
<script>
	$(function(){
	...
	function gobackIndex(){
		var url = '${request.contextPath}/scoremanage/scoreInfo/index/page?type=${type?default(0)}';
		$('.model-div').load(url);
	}
	function changeExam(){
	...
	function changeSubject(){
		...
		var type = $("#type").val();
		var selClass=$("#subjectInfoId");
		$.ajax({
			url:"${request.contextPath}/scoremanage/common/subjectListByGrade",
			data:{examId:examId,unitId:'${unitId!}',gradeCode:gradeCode,noLimit:'${noLimit!}'},
			dataType: "json",
			success: function(data){
				selClass.html("");
	...
	function searchList(data){
		...
		var acadyear=$("#acadyearSearch").val();
		var semester=$("#semesterSearch").val();
		var c2='?type=${type!}&acadyearSearch='+acadyear+'&semesterSearch='+semester+'&examId='+examId+'&gradeCode='+gradeCode+'&subjectInfoId='+subjectId+'&noLimit=${noLimit!}';
		var url='${request.contextPath}/scoremanage/scoreInfo/lock/page'+c2;
		$("#lockList").load(url);
	}
