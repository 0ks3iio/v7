...
<#if projectType?exists && projectType == '10'>
	...
	var url =  '${request.contextPath}/evaluate/query/statResult/page?evaluateType=${evaluateType!}&projectId=${project.id!}&subjectId=${subjectId!}&classId=${classId!}&teaId=${teacherId!}';
	<#else>
...
function searTable(){
	...
	var teaId = $('#teaId').val();
	if(teaId && teaId != ""){
		var url =  '${request.contextPath}/evaluate/query/statResultTxt/page?subjectId='+subjectId+'&teaId='+teaId+'&classId='+classId+'&evaluateType=${evaluateType!}&projectId=${project.id!}';
	}else{
