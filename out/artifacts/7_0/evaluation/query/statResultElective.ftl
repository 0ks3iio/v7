				...
				<table id="exampleElective" class="table table-bordered table-striped">
					...
					<#if dtolist?? && dtolist?size gt 0>
						...
						<tr style="height: 57px;">
							...
							<td class="noprint" ><a href="javascript:void(0)" onclick="toResultTxt('${dto1.teaId!}','${dto1.subId!}');">查看</a></td>
						</tr>
						</#list>
					</#if>
					</tbody>
...
function toResultTxt(teaId,subId){
	if(teaId && teaId != ""){
		var url =  '${request.contextPath}/evaluate/query/statResultTxt/page?teaId='+teaId+'&subId='+subId+'&evaluateType=${evaluateType!}&projectId=${project.id!}';
	}
	$(".model-div").load(url);
}
