				...
				<#if project.evaluateType == '11'>
					<div class="filter-item">
						...
						<div class="filter-content">
							<@popup.selectOneClass clickId="className" id="classId" name="className" handler="toClsTeaStat1()">
					...
					<div class="filter-item">
						...
						<div class="filter-content">
							<@popup.selectOneTeacher clickId="teaName" id="teaId" name="teaName" handler="toClsTeaStat1()">
					...
					<button class="btn btn-blue" onclick="doBatchExport();">批量导出</button>
					</div>
				</div>
				<#if itemType == '0'>
...
function doBatchExport(){
	var url =  '${request.contextPath}/evaluate/stat/statResult/exportZip?evaluateType=${evaluateType!}&projectId=${project.id!}';
...
function toClsTeaStat(){
	var url =  '${request.contextPath}/evaluate/stat/statTeaClsResult/page?evaluateType=${evaluateType!}&projectId=${project.id!}';
	$(".model-div").load(url);
}
function toTeaRankStat(){
	var url =  '${request.contextPath}/evaluate/stat/statTeaRankResult/page?evaluateType=${evaluateType!}&projectId=${project.id!}';
	$(".model-div").load(url);
}
...
<#if project.evaluateType == '11'>
	...
	var url =  '${request.contextPath}/evaluate/stat/statTeaClsResult/page?evaluateType=${evaluateType!}&projectId=${project.id!}&classId='+classId+'&itemType='+itemType;
	$(".model-div").load(url);
	<#else>
	var teaId = $('#teaId').val();
	var itemType = $('#itemType').val();
	var url =  '${request.contextPath}/evaluate/stat/statTeaClsResult/page?evaluateType=${evaluateType!}&projectId=${project.id!}&teaId='+teaId+'&itemType='+itemType;
	$(".model-div").load(url);
	</#if>
}
</script>
