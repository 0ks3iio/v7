            	...
            	<#assign courseScheduleKey='${tt}_${t}_${key!}'/>
                    ...
                    <#if (courseScheduleListMap[courseScheduleKey!])??>
                        ...
                        <#list courseScheduleListMap[courseScheduleKey!] as item>
                            <#if size gt 1><div></#if>
                                <#if showClass><b>${item.className!}</b><br/>${item.subjectName!}<br/><#else><b>${item.subjectName!}</b><br/></#if><#if showTeacher>${item.teacherName!}</#if>
                            <#if size gt 1></div></#if>
                        </#list>
                        </div>
                    </#if>
                        ...
                        <#list timetableCourseScheduleList as item>
                            <#if '${item.dayOfWeek!}_${item.period!}_${item.periodInterval!}'=='${tt}_${t}_${key!}'>
                                ...
                                <div class="infor"<#if bgc?default('') !='' || bdc?default('') != ''> style="<#if bgc?default('') !=''>background:${bgc};</#if><#if bdc?default('') !=''>border-color:${bdc};</#if>"</#if>>
                                    <b>${item.className!}</b><br/>${item.subjectName!}<br/>${item.teacherName!}<br/>
                                </div>
                            </#if>
                        </#list>
                    </#if>
        ...
        <tr>
            ...
            <#list 0..(edudays?default(7)-1) as tt>
                ...
                <#if (courseScheduleListMap[courseScheduleKey!])??>
                    ...
                    <#list courseScheduleListMap[courseScheduleKey!] as item>
                        <#if size gt 1><div></#if>
                            <#if showClass><b>${item.className!}</b><br/>${item.subjectName!}<#else><b>${item.subjectName!}</b></#if><br/><#if showTeacher>${item.teacherName!}</#if>
                        <#if size gt 1></div></#if>
                    </#list>
                    </div>
                </#if>
...
$(function(){
    var i = 1;
    $(".myplacetable .order").each(function(){
        $(this).text(i);
        i++;
    });
    var j = 1;
    $(".table-print .order").each(function(){
        $(this).text(j);
        j++;
    });
    $(".myplacetable td.item").each(function(){
