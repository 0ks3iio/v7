...
function initEventHandle() {
    ...
    websocket.onmessage = function (event) {
    	...
    	if (message.type == 'text') { 
             ...
             receiveIdText(id,text);
    	}else if(message.type =="div"){
    		var id =  message.id;
    		var url =  message.url;
    		url = _contextPath+url+"?cardId="+_cardId+"&view="+_view;
    		setTimeout(function(){
    			$("#"+id).load(url);
    		},2000);
...
function receiveIdText(id,text) {
	 if (id=="0010" && window.jsInterface){
   	 jsInterface.setOpenAndClose();
	 }else{
		 $("#"+id).html(text);
	 }
}
...
/* 第二部分：获取音频Audio,播放*/
var url = "https://tts.baidu.com/text2audio?lan=zh&ie=UTF-8&ctp=1&spd=5&vol=10&per=0";
var playAudio = new Audio();
function clockAudioPlay(cuid,token,str){
	url += "&cuid="+cuid;
	url += "&tok="+token;
	url += "&tex="+ encodeURI(str+"，，，。吧");
	playAudio.src = url;
	playAudio.play();
}
