...
<#if testResults?exists&&testResults?size gt 0>
<#list testResults as testResult>
			...
			<div class="box box-default">
				<div class="box-body clearfix">
					...
					<div class="margin-bottom-40">
						...
						<div class="font-16">
							<div id="result1${testResult_index+1}"></div>
							<div id="result2${testResult_index+1}"></div>
						</div>
					</div>
					<div class="margin-bottom-40">
						<div class="public-title-blue-border">得分分布</div>
						<div id="echarts${testResult_index+1}" style="width:250px;height:250px;"></div>
					</div>
					<div class="margin-bottom-40">
	...
	<#if testResult_index+1 != testResults?size>
		<div style="page-break-before:always;height:0px;">分页</div>
	</#if>
</#list>
</#if>
</div>
<script>
	$(function(){
		...
		<#if testResults?exists&&testResults?size gt 0>
		<#list testResults as testResult>
		result = "${testResult.answer!}";
		$("#result1"+${testResult_index+1}).text("1-30："+result.substring(0,59).replaceAll(',','、').replaceAll('1','是').replaceAll('2','否'));
		$("#result2"+${testResult_index+1}).text("31-60："+result.substring(60).replaceAll(',','、').replaceAll('1','是').replaceAll('2','否'));
		
			if(document.getElementById("echarts${testResult_index+1}")) {
				// 基于准备好的dom，初始化echarts实例
				var myChart${testResult_index+1} = echarts.init(document.getElementById("echarts${testResult_index+1}"));
				window.addEventListener('resize',function(){myChart${testResult_index+1}.resize();},false);
				// 指定图表的配置项和数据
				var option${testResult_index+1} = {
				    tooltip: {
				        trigger: 'axis'
				    },
				    radar: [
				...
				// 使用刚指定的配置项和数据显示图表
				myChart${testResult_index+1}.setOption(option${testResult_index+1});
			
			
			}
			</#list>
			</#if>
			<#if showprint?exists && showprint=="true">			
			setTimeout(function(){
				$('canvas').each(function(index){
					var url = $(this)[0].toDataURL();
					index += 1;
					$('#echarts'+index).html("<img src='" + url + "'>");
				});
				window.parent.printDetail();	
			},2000); 
			</#if>	
	});
	
	function getSubContent() {
		return getPrintContent(jQuery("#printDiv"));
	}
</script>