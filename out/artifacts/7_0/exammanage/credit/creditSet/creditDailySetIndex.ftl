...
<div class="layer-content">
    <div class="form-horizontal">
        <div class="form-group">
            <a class="btn btn-info btn-blue" onclick="addfen()">新增子项目</a>
        </div>
                            ...
                            <#if item.parentId?default("")?trim?length lt 1>
                                <#if item.subSetList?exists&& (item.subSetList?size > 0)>
                                    ...
                                    <tr data-id="${index1}">
                                        ...
                                        <#--<input type="hidden" id="subSetListIndex" value=${index}/>-->
                                            ...
                                            <#--<input type="hidden" name="dailySetList[${index}].score" value=${item.score!}>-->
                                        ...
                                        <#--<input type="hidden" id="subSetListIndex" value=${index}/>-->
                                            ...
                                            <input type="hidden" name="dailySetList[${index}].score" value=${item1.score!}>
                                            <input type="hidden" name="dailySetList[${index}].scoreType" value=${item1.scoreType!}>
                                        <#else >
                                            <input type="hidden" name="dailySetList[${index}].isParent" value="0">
                                            <#--<input type="hidden" id="subSetListIndex" value=${index}/>-->
                                            <input type="hidden" name="dailySetList[${index}].id" value=${item1.id!}>
                                    ...
                                    <#--<td>${item1.name!}</td>-->
                                        <td>
                                            ...
                                            <input class="form-control credit-control" nullable="false" name="dailySetList[${index}].name" type="text" value="${item1.name!}" style="width: 180px;"/>
                                            <i class="wpfont icon-plus credit-add" style="color:#317EEB"  id="${item1.parentId!}" onclick="addminscroe(this)"></i>
                                            <i class="fa fa-trash-o credit-delete" style="color:#317EEB" onclick="delminscroe(this)"></i>
                                        </td>
                                        <#if item1_index ==0>
                                ...
                                <tr data-id="${index1}">
                                    <td rowspan="1">
                                        <input type="hidden" name="dailySetList[${index}].isParent" value="1">
                                        <input type="hidden" name="dailySetList[${index}].id" value="${item.id!}">
                                        <input class="form-control credit-control" nullable="false" name="dailySetList[${index}].name" type="text" value="" style="width: 200px;" />
                                    </td>
                                    <#assign index = index + 1>
                                      <td>
                                          <input type="hidden" name="dailySetList[${index}].isParent" value="0">
                                          ...
                                          <input
                                                  class="form-control credit-control" nullable="false" name="dailySetList[${index}].name"
                                                  type="text"
                                                  value=""
                                                  style="width: 180px;"
                                          />
...
<script>
    ...
    function addminscroe(_this) {
        ...
        $(".layer-new").find("tr").each((i, n) => {
                    if ($(n).attr("data-id") == seindex) {
                        $(n).find("td").first().attr("rowspan", num + 1);
                        $(n).find("td").last().attr("rowspan", num + 1);
                        return false;
                    }
                });
    ...
    function delminscroe(_this){
        var seindex = $(_this)
                .parents("tr")
                .attr("data-id");
        var firstindex = false;
        if ($(_this).parents("tr").find("td").length < 3)
        {
            firstindex = true;
        ...
        if (firstindex) {
            $(".layer-new").find("tr").each((i, n) => {
                if ($(n).attr("data-id") == seindex) {
                $(n).find("td").first().attr("rowspan", num);
                $(n).find("td").last().attr("rowspan", num);
                return false;
            }
        });
        } else {
            ...
            if (islong > 1) {
                $(_this).parents("tr").find("td").eq(1).find("input").val($(_this).parents("tr").next().find("td").find("input").val());
    ...
    function delfen(_this) {
        // var jsonO = data;
        var seindex = $(_this).parents("tr").attr("data-id");
        $(".layer-new").find("tr").each((i, n) => {
            if ($(n).attr("data-id") == seindex) {
            $(n).remove();
        }
                    ...
                    if (jsonO.id) {
                        ...
                        $(".layer-new").find("table").append(`<tr data-id="` + dataid + `">
                          ...
                          <td>
                            ...
                            <i
                              class="fa fa-trash-o credit-delete"
                              style="color:#317EEB" onclick="delminscroe(this)"
                            ></i>
                          </td>
                          <td rowspan="1">
