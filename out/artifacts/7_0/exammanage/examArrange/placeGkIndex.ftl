...
<div class="tab-content">
	...
	<input type="hidden" name="examId" value="${examInfo.id!}"/>
		<div id="aa" class="tab-pane active" role="tabpanel">
			<table class="table table-bordered table-striped table-hover j-exaplacetable">
				<#if (emPlaceList?exists && emPlaceList?size>0)>
				<thead>
					<tr>
						<th >
						<label class="pos-rel">
							<input type="checkbox" id="placeCheckboxAll" class="wp" value="" onchange="placeCheckboxAllSelect()">
							<span class="lbl" style="font-weight: bold;">全选</span>
						</label>
						</th>
						<th class="">考试场地编号</th>
						<th class="">考试场地</th>
...
function placeCheckboxAllSelect(){
    if($("#placeCheckboxAll").is(":checked")){
        $('input:checkbox[name=placeCheckboxName]').each(function () {
...
$("#js-saveexaRoom").on("click", function(){	
	...
	<#--showConfirmMsg('如果考场考生已经生成，考试场地信息一旦发生改变，考场考生需要重排，请慎重操作！','提示',function(ii){
		layer.close(ii);
	});-->
	isSubmit=false;
	$("#js-saveexaRoom").removeClass("disabled");
