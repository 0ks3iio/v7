...
<div class="table-container">
	<div class="table-container-body" style="overflow-x: auto;">
		...
		<table class="table table-bordered table-striped table-hover">
			<thead>
				<tr>
					<th class="text-center" colspan="${(infoList?size*3+2)*linelist?size+1}">${title!}</th>
				</tr>
				<tr>
					<th rowspan="2">班级</th>
					<#list linelist as line>
					<th colspan="${infoList?size*3+2}">${line}</th>
					</#list>
				</tr>
				<tr>
				<#list linelist as line>
	 				...
	 				<tr>
					    ...
					    <#list linelist as line>
					    	<#if dtoMap1[line+'_'+cls.id]?exists>
								...
								<td>${dtoMap1[line+'_'+cls.id].rank!}</td>
							<#else>
								<td>0</td>
								<td>-</td>
							</#if>
							<#if infoList?exists && infoList?size gt 0>
								<#list infoList as info>
									<#if dtoMap2[info.id+'_'+cls.id+'_'+line]?exists>
										...
										<td>${dtoMap2[info.id+'_'+cls.id+'_'+line].num!}</td>
										<td>${dtoMap2[info.id+'_'+cls.id+'_'+line].rank!}</td>
									<#else>
										<td>-</td>
										<td>-</td>
										<td>-</td>
