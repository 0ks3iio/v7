<div id="aa" class="tab-pane active" role="tabpanel">
	<div class="col-md-3 no-padding-left">
		<div class="tree labels">
			<p class="tree-name no-margin">
				<b>信息列表</b>
				<#if canSend>
				<a href="javascript:void(0);" class="btn btn-blue btn-30" onclick="fullObjEdit('')">发布</a>
				</#if>
			</p>
			<#if eccFullObjs?exists&&eccFullObjs?size gt 0>
			<ul class="brand-message">
		        <#list eccFullObjs as item>
				<li onclick="fullObjShow('${item.id!}')"  <#if showId == item.id>class="bg-choose"</#if>>
	...
	<div class="col-md-9 no-padding-right">
		<div id="showMediaRight" class="tree labels">
			...
			<div class="no-data-container" style="padding-top:10px">
				<div class="no-data">
...
function fullObjEdit(id){
	var url =  '${request.contextPath}/eclasscard/standard/fullscreen/edit?id='+id+'&eccInfoId=${eccInfoId!}'+'&tabType=${tabType!}';
	$("#myEclasscardDiv").load(url);
}
function fullObjShow(id){
	if(id &&id!=''){
		var url =  '${request.contextPath}/eclasscard/standard/fullscreen/show?id='+id+'&eccInfoId=${eccInfoId!}'+'&tabType=${tabType!}';
		$("#showMediaRight").load(url);
	}
}

function fullObjDelete(id){
	layer.confirm("是否确认删除全屏展示该媒体内容", function(index){
		$.ajax({
			url:'${request.contextPath}/eclasscard/standard/fullscreen/delete',
			data: {'id':id},
			type:'post',
