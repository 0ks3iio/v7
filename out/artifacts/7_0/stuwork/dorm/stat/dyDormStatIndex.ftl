...
<script>
	...
	function doSearch(classId){
	    ...
	    var   url =  '${request.contextPath}/stuwork/dorm/stat/list?classId='+classId+"&"+searchUrlValue("#searchType");
        $("#showList").load(url);
	}
	function changeWeek(){
		...
		$.ajax({
			url:'${request.contextPath}/stuwork/dorm/stat/setWeekList',
	...
	function saveStat(){
		...
		$.ajax({
			url:'${request.contextPath}/stuwork/dorm/stat/save',
