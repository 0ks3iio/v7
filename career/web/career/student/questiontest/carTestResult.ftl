...
<div class="page-content">
		...
		<div class="col-xs-12">
			<div class="box box-default">
				<div class="box-body clearfix">
					...
					<div>
						<div class="public-title-blue-border">模型简介</div>
						<button class="btn btn-white btn-sm" id="showhld" onClick="showHLD()">展开查看详情</button>
						<div id="hldDiv" class="none" style="display:none">
	...
	<div class="navbar-fixed-bottom opt-bottom">
		<a class="btn btn-blue" href="#" onClick="printPDF()">导出pdf</a>
		<a class="btn btn-white" href="#" onClick="printDetail()">打印</a>
		<a class="btn btn-white" href="#" onClick="tryAgain()">重新测评</a>
	</div>
</div><!-- /.page-content -->
...
<script>
	...
	function showHLD() {
		if ($("#hldDiv").attr("class") == "none") {
			$("#hldDiv").attr("style","display:block");
			$("#hldDiv").attr("class","block");
			$("#showhld").text("收起详情");
		} else {
			$("#hldDiv").attr("style","display:none");
			$("#hldDiv").attr("class","none");
			$("#showhld").text("展开查看详情");
		}
	}
	
	function tryAgain() {
		var url = "${request.contextPath}/careerplan/student/question/list";
		$("#carPage").load(url);
	}
	...
	function switchImg() {
		if (!isImage) {
			var canvas = $('#echarts canvas');
    		var url = canvas[0].toDataURL();
    		$('#echarts').html("<img src='" + url + "'>");
			isImage = true;
		}
	}
	...
	function printPDF() {
		layerTime();
		$.ajax({
			url:'${request.contextPath}/careerplan/student/testresult/printPDF',
			type:'post',
	...
	function exportPdf(){
		...
		if(carplanform){
			carplanform.action="${request.contextPath}/careerplan/student/testresult/downPDF";
			carplanform.submit();
		}
		layerClose();
	}
	...
	function printDetail(){
		...
		switchImg();
		$("#hldDiv").attr("style","display:block");
		$("#showhld").attr("style","display:none");
		...
		LODOP.ADD_PRINT_HTM("20mm","15mm","RightMargin:15mm","BottomMargin:15mm",getPrintContent($("#printDiv")));
