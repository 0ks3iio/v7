		...
		<div class="table-container-body">
			...
			<#list piMap?keys as piFlag>
			    ...
			    <#if pIndex == 1>
					...
					<#list 0..weekDays as day>
					<td class="text-center edited <#if arrayItemId?default('') == '' && day gt 4>active</#if>" td-num="${editNum*(weekDays+1) + day}"></td>
					</#list>
			    </tr>
...
$(function(){
	...
	// 刷新课时计数器
	function refreshCounter(c){
		$('.schedule-label-2').find('.badge').text(c);
	}
	...
	$('#table01 .edited').on('click', '.del', function(e){
		e.stopPropagation();
		if($(this).siblings('.label-timeselect').length>0){
			refreshCounter(--counter);
		}
		$(this).parent().remove();
	});
	...
	function imitateClick(lessonInfJson){
		...
		$.each(lessonInfJson,function(i,e){
			...
			if($("."+classNameStr)){
				...
				if($(tdobj).find('.label-timeselect').length>0){
					refreshCounter(++counter);
				}
			}
