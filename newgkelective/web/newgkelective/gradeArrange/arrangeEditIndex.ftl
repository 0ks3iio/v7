<script>
	...
	function backToGradeList(){
		...
		<#elseif arrayId?default('')==''>
		   ...
		   var url =  '${request.contextPath}/newgkelective/'+gradeId+'/goArrange/addArray/page?divideId=${divideId!}&lessArrayId='+array_item_id+'&plArrayId=${plArrayId!}';
			$("#showList").load(url);
	    <#else>
...
<#if arrayItemId?default('')==''>
	...
	function gradeTable(){
		var array_item_id = $('[name="array_item_id"]').val();
		var url = "${request.contextPath}/newgkelective/${divideId!}/gradeSet/edit?arrayItemId="+array_item_id;
	...
	function subjectTable(){
		...
		var url = '${request.contextPath}/newgkelective/'+array_item_id+'/courseFeatures/index';
		$("#gradeTableList").load(url);
	}
	function classTable(cid2){
		//debugger;
		var array_item_id = $('[name="array_item_id"]').val();
		if(!checkItemId(array_item_id)){
			return;
