...
<div class="layer layer-setting-tip layer-searchGroups">
	<div class="layer-content">
		<div class="publish-course publish-course-sm">
			<#if groupMap?exists && (groupMap?size>0)>
				<#list groupMap?keys as key>
					<span class="groupIdsearch <#if zeroStrs?default('')?index_of(key) != -1> disabled<#else><#if groupsubjectIds?index_of(key) gt -1 > active</#if></#if>" data-value="${key!}" style="min-width:90px;width:auto;">${groupMap[key]!}</span>
				</#list>
			</#if>
		</div>
	</div>
			...
			<div class="table-switch-filter">
				<div class="filter filter-sm">
					<div class="filter-item" style="margin-right:10px;">
						<a href="javascript:"  class="btn btn-sm btn-blue js-searchGroups">选择组合</a>
					</div>
					<div class="filter-item" style="margin-right:5px;">
						<input type="hidden" name="groupIds" id="groupIds" value="${groupsubjectIds?default('')}">
...
<script>
	...
	<#--加载左边-->
	function loadLeft(){
		var groupSubjectIds=$("#groupIds").val();<#--多个组合选择-->
		var url =  '${request.contextPath}/newgkelective/${divideId!}/divideGroup/schedulingLeft2/page?groupSubjectIds='+groupSubjectIds;
	...
	function openSomeClass(){
		...
		showConfirm("是否确定根据系统规则进行开班",options,function(){
			...
			$.ajax({
				...
				data:{"subjectIds":subjectIds,"groupSubjectIds":groupIds,"openNum":rows,"stuids":stuIdStr},
				dataType : 'json',
				type:'post',
				success:function(data) {
					var jsonO = data;
	...
	$(function(){
		...
		$('.js-searchGroups').on('click', function(e){
			e.preventDefault();
			layer.open({
				type: 1,
				shadow: 0.5,
				...
				btn2:function(){
					var oldGroupIds=$("#groupIds").val();
					if(oldGroupIds==""){
						$(".groupIdsearch").removeClass("active");
					}else{
						<#--还原选项-->
						$(".groupIdsearch").removeClass("active");
						$(".groupIdsearch").each(function(){
							var vv=$(this).attr("data-value");
							if(oldGroupIds.indexOf(vv)>-1){
				...
				content: $('.layer-searchGroups')
			})
		});
		$(".groupIdsearch").on('click',function(e){
			e.preventDefault();
	...
	function refeshAll2(){
		 var subjectIds1=$("#groupId").val();
		 var groupClassId1=$("#groupClassId").val();
		 refeshAll(subjectIds1,groupClassId1);
	}
	
	
	function refeshAll(subjectIds,divideClassId){
		var groupsubjectIds=$("#groupIds").val();
		var url =  '${request.contextPath}/newgkelective/${divideId!}/divideGroup/groupDetail/page?subjectIds='+subjectIds+"&groupClassId="+divideClassId+"&groupsubjectIds="+groupsubjectIds;
		$("#showList").load(url);
	}
