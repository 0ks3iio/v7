...
function toBack(){
	<#if divide.openType?default('')=='06' || divide.openType?default('')=='02'>
		$("#showList").load("${request.contextPath}/newgkelective/clsBatch/${divide.id!}/index");
	<#else>
		$("#showList").load("${request.contextPath}/newgkelective/BathDivide/${divide.id!}/arrangeAList/page");
	</#if>
}
...
function autoAllClass3(divideId,indexStr){
    ...
    var urlStr="${request.contextPath}/newgkelective/BathDivide/"+divideId+"/openClassArrange/saveBathOnlyB";
