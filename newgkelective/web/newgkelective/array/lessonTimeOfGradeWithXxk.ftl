<div id="aa" class="tab-pane active" role="tabpanel">
	<div class="explain explain-white">
		...
		<#list dtos as dto>
			...
			</#if><#if !(isXzbArray!)> ；共计${dto.period?default(0)}课时</#if></div>
		</#list>
		</#if>
		<div class="color-999"><i class="glyphicon glyphicon-info-sign color-yellow"></i>
		<#if isXzbArray!>
			 小提示：各走班时间点的最佳设置数等于${xuanPeriod?default(0)}，才有可能排课成功；
		<#else>
			 小提示：选考各时间点的最佳设置数等于${xuanPeriod?default(0)}，学考各时间点的最佳设置数等于${xuePeriod?default(0)}，才有可能排课成功；选考、学科各自时间点的设置数应一致
		</#if>
		</div>
	</div>
	<div class="row">
	...
	<div class="navbar-fixed-bottom opt-bottom">
		<#if !(isXzbArray!)><a src="javascript:void(0);"  class="btn btn-blue" id="autoArrange">自动安排</a></#if>
...
$(function(){
	...
	function imitateClick(lessonInfJson){
		...
		$.each(lessonInfJson,function(i,e){
			...
			var tdtype = document.getElementById("type"+objId).value;
			//var tdlabel = $('#data'+tdtype);
			var tdlabel = document.getElementById("data"+tdtype);
