...
<div class="navbar-fixed-bottom opt-bottom">
		<button class="btn btn-blue"id="confirmSubmit3">确定</button>
...
$(function(){
	...
	$(function(){
		...
		var url = "${request.contextPath}/newgkelective/"+array_item_id+"/subjectTimeinf/json?groupType=${groupType!}";
		$.post(url, 
			function(data){
	...
	function saveGrade3(){
		...
		hasSubmit=true;
		var array_item_id = $("input[name='array_item_id']").val();
		var url = '${request.contextPath}/newgkelective/'+array_item_id+'/updateLessonTimeNew?objectType=${objectType!'9'}';
	...
	//总课表提交结果  .btn-blue:first
	$('#confirmSubmit3').click(function(){
		layer.confirm("确认提交结果吗？",function(index){
				saveGrade3();
			}
		);
	});
