<#--
	<div class="page-toolbar">
		<a href="javascript:goBack1();" class="page-back-btn""><i class="fa fa-arrow-left"></i> 返回</a>
...
<div class="filter">	
	...
	<div class="filter-item filter-item-right">
		<span class="color-999">共有 <#if dtoMap?exists>${dtoMap?size!0}<#else>0</#if>条结果</span>
	</div>
</div>
		...
		<div class="number-container">
			<a href="javascript:void(0);" onClick="doUpdate('${key!}');">
				<ul class="number-list">
					<#if dtoMap[key].teacherPlanList?exists && dtoMap[key].teacherPlanList?size gt 0>
					<#list dtoMap[key].teacherPlanList as item>
					<#if item.exTeacherIdList?exists && item.exTeacherIdList?size gt 0 >
...
<script>
	showBreadBack(goBack1,false,"教师资源列表");
	function goBack1(){
		<#if arrayId?default('')==''>
	...
	function doDelete(itemId){
		...
		layer.confirm('确定删除吗？', function(index){
			$.ajax({
				url : '${request.contextPath}/newgkelective/'+itemId+'/subjectTeacherArrange/delete',
				dataType : 'json',
				type : 'post',
				success : function(data){
			 		if(data.success){
	                    ...
	                    url =  '${request.contextPath}/newgkelective/${divideId}/teacherArray/index/page?gradeId=${gradeId}&arrayId=${arrayId}';
	                    $("#showList").load(url);
			 		}
			 		else{
	...
	function doUpdate(itemId){
		var url='${request.contextPath}/newgkelective/${divideId!}/subjectTeacherArrange/add?gradeId=${gradeId!}&itemId='+itemId+'&arrayId=${arrayId!}';
		$("#showList").load(url);
	}
	function doAdd(){
		var url='${request.contextPath}/newgkelective/${divideId!}/subjectTeacherArrange/add?gradeId=${gradeId!}&arrayId=${arrayId!}';
		$("#showList").load(url);
	}
</script>