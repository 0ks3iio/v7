...
<div class="filter">	
	<div class="filter-item">
		...
		<a href="javascript:" class="btn btn-white" onclick="mutilArray()">多年级排课</a>
	</div>
		...
		<#else>
			...
			<#else>
				<ul class="number-list number-list-big">
					<li>
						<a href="javascript:" onclick="resultView('${item.id!}','1')"><div class="number-icon number-icon01"><i>行</i></div><span>行政班结果</span></a>
					</li>
					<li>
						<a href="javascript:" onclick="resultView('${item.id!}','2')"><div class="number-icon number-icon02"><i>教</i></div><span>教学班结果</span></a>
					</li>
					<li>
					...
					<li>
						<a href="javascript:" onclick="resultView('${item.id!}','10')"><div class="number-icon number-icon04"><i>换</i></div><span>学生选课调整</span></a>
					</li>
					</#if>
				</ul>
			</#if>
...
<script type="text/javascript"> 
	...
	function addArray(){
		...
		isAddArr=true;
		clearTimeout(setTimeClick);
		var url =  '${request.contextPath}/newgkelective/xzb/addArray/page';
		$("#showList").load(url);
	}
	var isShowSet=false;
	...
	function refreshList(pageIndex, pageSize,arrayId){
		...
		//var ii = layer.load();
		var url =  '${request.contextPath}/newgkelective/xzb/index/list/page?arrayId='+arrayId+'&useMaster='+useMaster;
		if(pageIndex){
	...
	function mutilArray(arrayId){
		if(!arrayId){
			arrayId = "arrayId";
		}
		var url = '${request.contextPath}/newgkelective/xzb/'+arrayId+'/mutilArray/select';
		indexDiv = layerDivUrl(url,{title: "选择排课方案",width:820,height:600});
	}
