<div class="filter">
	<div class="filter-item">
		<a href="javascript:void(0)" class="btn btn-blue js-shortcuts">选/学考课时设置</a>
		
		<a href="javascript:void(0)" class="btn btn-white js-courseAdd">新增科目</a>
	</div>
	<div class="filter-item">
		<b class="float-left mt3" style="font-size:15px;">最大课时数：<span id="lecCount">0</span></b>
	</div>
</div>
		...
		<table class="table table-bordered table-striped table-hover" id="subjectTable">
			<thead>
				<tr>
					...
					<#if isZugd?default(false)>
						<th rowspan="2">是否按照<br>组合班上课</th>
					</#if>
					<th rowspan="2">操作</th>
				</tr>
				<tr>
			...
			<tbody>
				...
				<#assign indexNumber=dto_index>
 				<tr>
				    <td>
				    <input type="hidden" name="subjectIdType" class="subjectIdType" value="${(dto.subjectId + '-' + dto.subjectType)!}">
					<input type="hidden" name="dtoList[${indexNumber}].subjectId" class="subjectId" value="${dto.subjectId!}">
				    ...
				    <td>
				    	<div class="form-number  form-number-sm period-number" data-step="1">
				    		<input type="text" name="dtoList[${indexNumber}].courseWorkDay" class="form-control courseWorkDay"  value="${dto.courseWorkDay?default(0)}" onblur="changeInputNum(this)"/>
				    ...
				    <td>
				    	...
				    	<select name="dtoList[${indexNumber}].firstsdWeekSubjectId" class="form-control firstsdWeekSubjectId" <#if (dto.courseWorkDay?default(0)) == 0>disabled='disabled'</#if> onChange='changeFirstsdWeek(this)'>
				    ...
				    <td>
				    	<div class="form-number form-number-sm courseCoupleTimes-number" data-step="1" >
				    		<#if (dto.courseWorkDay?default(0)) gt 1>
				    			<input type="text" name="dtoList[${indexNumber}].courseCoupleTimes" class="form-control courseCoupleTimes" value="${dto.courseCoupleTimes?default(0)}" onblur="changeInputNum(this)"/>
					    		<button class="btn btn-sm btn-default btn-block form-number-add"><i class="fa fa-angle-up"></i></button>
					    		<button class="btn btn-sm btn-default btn-block form-number-sub"><i class="fa fa-angle-down"></i></button>
				    		<#else>
				    			<input type="text" name="dtoList[${indexNumber}].courseCoupleTimes" class="form-control courseCoupleTimes" disabled value="0" onblur="changeInputNum(this)"/>
				    ...
				    <td>
				    	<select name="dtoList[${indexNumber}].arrangePrior" class="form-control arrangePrior" style="min-width:90px;" onChange='changeFunctionOpo(this)'>
				    ...
				    <#--td>
				    	<label>
							<input type="checkbox" name="dtoList[${indexNumber}].punchCard" value="1" class="wp wp-switch" <#if (dto.punchCard!1) == 1>checked</#if> onclick="changeFunctionOpo(this)">
							<span class="lbl"></span>
						</label>
				    </td-->
				    <td>
					...
					<#if isZugd?default(false)>
						<td>
							<label>
								<input <#if dto.subjectType! != "O">disabled</#if> type="checkbox" name="dtoList[${indexNumber}].followZhb" value="1" class="wp wp-switch"
										<#if (dto.followZhb!0) == 1>checked</#if> onclick="changeFunctionOpo(this)">
								<span class="lbl"></span>
							</label>
						</td>
					</#if>
...
<#-- 快捷设置 -->
<div class="layer layer-shortcuts">
	<div class="layer-content shortcuts-div">
		<table width="100%">
			<tr>
				<td width="90"><p class="mb10">选考周课时：</p></td>
				<td>
					<input type="text" name="" vtype="int" id="periodA" maxlength="2" class="form-control mb10">
				</td>
			</tr>
			<tr>
				<td><p class="mb10">学考周课时：</p></td>
				<td>
					<input type="text" name="" vtype="int" id="periodB" maxlength="2" class="form-control mb10">
				</td>
			</tr>
		</table>
	</div>
...
<div class="layer layer-courseAdd">
	<div class="layer-content courseAdd-div">
		...
		<table class="" width="100%">
			<tr>
				<td class="text-right" width="100"><p>选考科目：</p></td>
				<td class="course-A">
					<#if gkCourseList?exists && gkCourseList?size gt 0>
						<#list gkCourseList as course>
						<label class="mb10 mr10">
						<input type="checkbox" class="wp" value="${course.id!}-A" onclick="checkChange(this)">
			...
			<tr>
				<td class="text-right" valign="top"><p>学考科目：</p></td>
				<td class="course-B">
					<#if gkCourseList?exists && gkCourseList?size gt 0>
						<#list gkCourseList as course>
						<label class="mb10 mr10">
						<input type="checkbox" class="wp" value="${course.id!}-B" onclick="checkChange(this)">
						<span class="lbl"> ${course.subjectName!}</span>
						</label>
						</#list>
					</#if>
			...
			<tr>
				<td class="text-right" valign="top"><p>行政班科目：</p></td>
				<td class="course-O">
...
<script>
var parMap = {
<#if parMap?? && parMap?size gt 0>
<#list parMap?keys as pid>
<#if pid_index !=0>,</#if>
"${pid!}":[<#list parMap[pid] as cid><#if cid_index !=0>,</#if>"${cid!}"</#list>]
</#list>
</#if>};

	var opoNum=0;//操作次数
	var isZugd = <#if isZugd?default(false)>true<#else>false</#if>;//分班是否为组合固定模式
	...
	$(function(){
		...
		$('.js-shortcuts').on('click', function(){
			$(".shortcuts-div").find("input[type='text']").val("");
			layer.open({
				type: 1,
				shadow: 0.5,
				title: '选/学考课时设置',
				...
				btn1:function(){
					opoNum++;
 					if(!checkValue(".shortcuts-div","")){
 						
 					}else{
 						var periodA=$("#periodA").val().trim();
 						var periodB=$("#periodB").val().trim();

 						$("#subjectTable tbody").find("tr").each(function(){
							var keyId=$(this).find("input[name='subjectIdType']").val();
							if(keyId.indexOf('-A') >-1){
								if(periodA!=""){
 									$(this).find(".courseWorkDay").val(periodA);
 								}
							}else if(keyId.indexOf('-B') >-1){
								if(periodB!=""){
									$(this).find(".courseWorkDay").val(periodB);
								}
		...
		$('.js-courseAdd').on('click', function(){
			...
			<#if false && notBasic?default('')=='1'>
				$(".course-A").find("input[type='checkbox']").each(function(){
					$(this).attr("disabled",true);
				})
				$(".course-B").find("input[type='checkbox']").each(function(){
					$(this).attr("disabled",true);
				})
	...
	function saveMySubjectTime(){
		...
		isSubmit=true;
		if(!checkSubjectIdParm() || !checkChildren()){
			isSubmit=false;
		 	return;
		}
		...
		var url;
		<#if notBasic?default('')!='1'>
		url = '${request.contextPath}/newgkelective/${gradeId!}/courseFeatures/courseFeaturesSave';
		<#else>
		url = '${request.contextPath}/newgkelective/${arrayItemId!}/courseFeatures/courseFeaturesSave';
		</#if>
		var options = {
			url : url,
			dataType : 'json',
	...
	// 检查拆分出的课程的课时数是否超过了 被拆分的课程课时数 
	function checkChildren(){
		var f=true;
		$("#subjectTable tbody tr").each(function(i,obj){
			var subjectId = $(obj).find(".subjectId").val();
			var subjectType = $(obj).find(".subjectType").val();
			var period = parseInt($(obj).find(".courseWorkDay").val());
			if(parMap[subjectId]){
				var cids = parMap[subjectId];
				var sum = 0;
				//debugger;
				for(x in cids){
					var cid = cids[x];
					var $cidType = $('#subjectTable tbody .subjectIdType[value="'+cid+'-'+subjectType+'"]');
					//console.log($cidType.length);
					var co = parseInt($cidType.parents("tr").find(".courseWorkDay").val());
					if(co) sum += co;
				}
				if(sum > period){
					layer.tips("拆分的科目课时数不能超过被拆分的课时数！",$(obj).find('.courseWorkDay'), {
						tipsMore: true,
						tips: 3
					});
					f = false;
	...
	function toRefesh(useMaster){
		...
		<#else>
		url = '${request.contextPath}/newgkelective/${arrayItemId!}/courseFeatures/index?useMaster='+useMaster;
		</#if>
...
function deleteSubject(obj){
	...
	layer.confirm('将会直接删除生效,确定删除吗？', {
	  btn: ['确定','取消'] //按钮
		...
		<#if notBasic?default('')!='1'>
	  	var url = '${request.contextPath}/newgkelective/${gradeId!}/courseFeatures/deleteSubject';
		<#else>
	...
	$("#subjectTable tbody tr").each(function(){
		...
		if(workDay > 0){
			...
			if(subType == "O"){
				...
				console.error("未知的科目类型："+subType);
			}
		}
		if((subType == "A" || subType == "B") && abSubIds.indexOf(subId) == -1){
			abSubIds.push(subId);
		}
	});
	
	var sortor = function(a,b){return b-a;}
	aNums.sort(sortor);
	bNums.sort(sortor);
	
	var total = xzbNum;
	for(var x=0;x<3&&x<aNums.length;x++){
		total += aNums[x];
	}
	//debugger;
	var openNumB = 3;
	if(abSubIds.length > 6){
		openNumB = 4;
	}
	for(var x=0;x<openNumB && x<bNums.length;x++){
		total += bNums[x];
	}
	$("#lecCount").text(total);
}
