...
<div class="box box-default" id="parmA">
	...
	<form id="editForm" name="editForm" method="post" >
		<div class="box-body">	
					...
					<table class="table table-bordered" style="margin-top:10px">
						<tbody>
							<tr>
								...
								<th width="20%">班级数</th>
								<th width="">每班人数（浮动值）
									<input style="width:20%;<#if !canEdit?default(true)>border: 1px solid #ccc;background-color: #eee;</#if>" type="text" id="bFloNumId" onkeydown="dispSetFloNum(this)" <#if !canEdit?default(true)>readOnly="readOnly"</#if>>
									<a type="button" class="btn btn-default <#if !canEdit?default(true)>disabled</#if>" onclick="setFloNum(this)">批量赋值</a>
								</th>
							</tr>
							<#list jxbList as dto>
			...
			<#if (jxbList?exists && jxbList?size gt 0) || (xzbList?exists && xzbList?size gt 0)>
...
function saveGroup(){
	...
	// 提交数据
	var options = {
	    url:'${request.contextPath}/newgkelective/${divideId!}/singleList/save',
	    dataType : 'json',
	    type:'post',  
	    cache:false, 
