			...
			<div class="box-body">
				...
				<h4 class="form-title">
					...
					<label class="pos-rel color-blue font-12 no-margin">
						<input id="isFollow" type="checkbox" class="wp" name="isFollow"  <#if !(followType?default('B-1')?index_of('B-0') gt -1)>checked</#if> <#if !canEdit>disabled <#else> onchange="switchFollow()" </#if>>
					...
					<div class="col-xs-6">
						...
						<div class="single-subject num-input" groupIndex="${dto_index+jxbSize}">
							...
							<div class="bd">
								...
								<div class="follow" <#if (followType?default('B-1')?index_of('B-0') gt -1)>style="display: none"</#if>>
								...
								<div class="filter mt25 unFollow divide-filter-block" id="" <#if !(followType?default('B-1')?index_of('B-0') gt -1)>style="display: none"</#if>>
									<div class="filter-item">
										<span class="filter-name">分班数：</span>
										<div class="filter-content">
											<div class="input-group form-num form-num-sm float-left w70" style="width: 80px">
												<input class="form-control divide-class-num" type="text" onblur="refreshDivideClassNum(this)" nullable="false" maxlength="2" <#if !canEdit>readOnly="readOnly"</#if> vtype="int" max="${dto.stuNum?default(1)}" min="0" name="exList[${dto_index+jxbSize}].classNum" value="${ex.classNum?default('')}">
									...
									<div class="filter-item no-margin">
										...
										<div class="filter-content">
											<div class="input-group form-num form-num-sm float-left w70">
												<input id="${dto_index+jxbSize}_aveNum" class="form-control divide-filter-num" nullable="false" maxlength="3" <#if !canEdit>readOnly="readOnly"</#if> vtype="int" max="${dto.stuNum?default(1)}" min="0" type="text" <#if anum?default(-1)!=-1>value="${anum?string('0')}"</#if>>
												<span class="input-group-btn">
				                                    <button class="btn btn-default count-up" type="button">
				                                    	<i class="fa fa-angle-up"></i>
				                                    </button>
											...
											<div class="input-group form-num form-num-sm float-left w70">
												<input id="${dto_index+jxbSize}_floNum" class="form-control" nullable="false" maxlength="2" <#if !canEdit>readOnly="readOnly"</#if> vtype="int" max="${dto.stuNum?default(1)}" min="0" type="text" <#if bnum?default(-1)!=-1>value="${bnum?string('0')}"</#if>>
												<span class="input-group-btn">
				                                    <button class="btn btn-default count-up" type="button">
				                                    	<i class="fa fa-angle-up"></i>
				                                    </button>
							...
							<div class="bd">
								...
								<div class="filter mt25 divide-filter-block">
									...
									<div class="filter-item no-margin">
										...
										<div class="filter-content">
											<div class="input-group form-num form-num-sm float-left w70">
				                                <input id="${dto_index}_aveNum" class="form-control divide-filter-num" maxlength="3" <#if !canEdit>readOnly="readOnly"</#if> nullable="false" vtype="int" max="${dto.stuNum?default(1)}" min="0" type="text" <#if anum?default(-1)!=-1>value="${anum?string('0')}"</#if>>
				                                <span class="input-group-btn">
				                                    <button class="btn btn-default count-up" type="button">
				                                    	<i class="fa fa-angle-up"></i>
				                                    </button>
				                            ...
				                            <span class="float-left ml5 mr5 mt3">±</span>
				                            <div class="input-group form-num form-num-sm float-left w70">
				                                <input id="${dto_index}_floNum" class="form-control" maxlength="2" <#if !canEdit>readOnly="readOnly"</#if> nullable="false" vtype="int" max="${dto.stuNum?default(1)}" min="0" type="text" <#if bnum?default(-1)!=-1>value="${bnum?string('0')}"</#if>>
				                                <span class="input-group-btn">
				                                    <button class="btn btn-default count-up" type="button">
				                                    	<i class="fa fa-angle-up"></i>
				                                    </button>
...
<div id="beforeNext" style="display: none">
	<div class="filter-item block" style="margin-top: 20px; margin-left: 30px">
		<div id="followDiv">
			<div class="filter-content">
				<input type="radio" class="followType" name="followType_1" value="B-1" checked>&nbsp;走班时间上课
			</div>
			<div class="filter-content">
				<input type="radio" class="followType" name="followType_1" value="B-2">&nbsp;行政班时间上课
			</div>
		</div>
	</div>
