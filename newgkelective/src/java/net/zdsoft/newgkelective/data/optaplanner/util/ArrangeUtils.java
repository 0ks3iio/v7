...
//		List<String[]> a = arrangementSelectString(new String[] { "1", "2", "3", "4" }, 4);

		List<List<Integer>> combinationSelect = combinationSelect(new Integer[] { 1, 2, 3, 4, 5 }, 3);
		System.out.println(combinationSelect);
	}
	 ...
	 *            选择索引，从0开始
	 */
	public static void arrangementSelectString(String[] dataList, String[] resultList, int resultIndex,
			List<String[]> list) {
	 ...
	 *            选择索引，从0开始
	 */
	public static void arrangementSelectInteger(Integer[] dataList, Integer[] resultList, int resultIndex,
			List<Integer[]> list) {
		int resultLen = resultList.length;
		if (resultIndex >= resultLen) { // 全部选择完时，输出排列结果
	 ...
	 * @return 
	 */
	public static <T> List<List<T>> combinationSelect(T[] dataList, int n) {
//		System.out.println(String.format("C(%d, %d) = %d", dataList.length, n, combination(dataList.length, n)));
		
		return combinationSelect(dataList, 0, (T[])new Object[n], 0,null);
	}
	 ...
	 *            选择索引，从0开始
	 */
	private static <T> List<List<T>> combinationSelect(T[] dataList, int dataIndex, T[] resultItem, int resultIndex, List<List<T>> resultList) {
		if(resultList == null) {
			resultList = new ArrayList<>();
		}
		
		int resultLen = resultItem.length;
		int resultCount = resultIndex + 1;
		if (resultCount > resultLen) { // 全部选择完时，输出组合结果
			List<T> itemList = Arrays.stream(resultItem).collect(Collectors.toList());
//			System.out.println(itemList);
		...
		for (int i = dataIndex; i < dataList.length + resultCount - resultLen; i++) {
			resultItem[resultIndex] = dataList[i];
			combinationSelect(dataList, i + 1, resultItem, resultIndex + 1, resultList);
		}
