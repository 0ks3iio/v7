...
public class NKRedisLock {
    ...
    public NKRedisLock(boolean fair, int blockingAfterLockNum, long parkThreadMilliscond) {
        this.fair = fair;
    ...
    public void lock(String key, int keyExpireSecond) {
        ...
        do {
            ...
            if (val == null) {
                RedisUtils.set(key, String.valueOf(System.currentTimeMillis()), keyExpireSecond);
                return;
            } else {
                if(!isSaveLock) {
                    lockThread = saveLock(key);//保证一个key对应一个lock
    ...
    public boolean resetTTL(String key, int seconds) {
    ...
    public void unlock(String key) {
        globalLock.lock();
        RedisUtils.del(key); //必须先删除redis的key
