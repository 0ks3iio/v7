...
public class ExcelFileDataLoader3A {
	private char					majorOrMinor; 			//input: 'A': 选考课， 'B': 学考课
	...
	public ExcelFileDataLoader3A(String excelFileName, char aOrB) {
		
		majorOrMinor = aOrB;
		//surplusTimeSlotCount = timeSlotCount;
		...
		//设置每个TimeSlot的preStudentCourseMap
	...
	private void populateStudentList(String excelFileName) {
		...
		for (StudentVO3 rawStudent : rawStudentList) {
			...
			if (myACourseNameSet.size() != 3) {
				System.out.println("ERROR: Dirty Data! " + rawStudent.getName() + ", " + rawStudent.getStuId());
			}
			Set<String> myBCourseNameSet = Sets.difference(courseNameSet, myACourseNameSet);
			
			Set<String> courseNameSetTOGO = (majorOrMinor == 'A' || majorOrMinor == 'a') ? myACourseNameSet : myBCourseNameSet;
			
			for (String courseName : courseNameSetTOGO) {
				StudentCourse studentCourse = new StudentCourse();
				TakeCourse currentCourse = courseMap.get(courseName);
	...
	private void populateTimeSlotList() {
		...
		if (majorOrMinor == 'B' && courseList.size() > 6) { //大于6的时候就是7选3了
			timeSlot = new TimeSlot();
			timeSlot.setId(id);
			timeSlot.setTimeSlotName("T" + id);
			timeSlotList.add(timeSlot);
	...
	private void populateCourseSectionSize() {
		...
		for(TakeCourse course : courseList) {
			...
			//courseSectionSizeMarginMap.put(courseName, 2);
