...
import net.zdsoft.syncdata.custom.yingshuo.service.YingShuoService;
...
@Service("yingShuoService")
public class YingShuoServiceImpl implements YingShuoService {
	...
	public void saveEdu() {
		...
		String eduCode = getEduCode();
		if(StringUtils.isNotBlank(eduCode)){
			String url = getUrl(YingShuoConstant.YS_GET_EDU_UNIT_URL, "fkcode", eduCode);
			String cc = getDate (url,StringUtils.EMPTY,Boolean.TRUE);
			JSONObject js = Json.parseObject(cc);
			String id = doChangeId(eduCode);
			String parentId = Unit.TOP_UNIT_PARENT_GUID_NOT_TRUE;
			String unitName = js.getString("edu_name");
			Integer isDeleted = Integer.valueOf(js.getString("del_status"));
			String regionCode = js.getString("district_code");
			String linkPhone = js.getString("tel");
			Date creationTime = new Date();
			Date modifyTime =  DateUtils.string2DateTime(js.getString("update_time"));		
//			----------封装单位
			Unit unit = SUtils.dc(unitRemoteService.findOneById(doChangeId(eduCode)), Unit.class);
			if(unit == null){
				unit = new Unit();
	...
	public void saveSchool() {
		try {
			List<Unit> regionUnits = SUtils.dt(unitRemoteService.findByRegionCode(YingShuoConstant.YS_REGION_CODE_VAL),
					new TR<List<Unit>>() {});
...
//			}
			...
			Map<String, StusysSectionTimeSet> timeMap = EntityUtils.getMap(times, StusysSectionTimeSet::getId);
//			TODO 根据教育据code 来获取学校信息 ，改为 根据 用户信息来获取 
			String cc = getDate (YingShuoConstant.YS_GET_USER_LIST_URL,StringUtils.EMPTY,Boolean.TRUE);
			String eduCode = YingShuoConstant.YS_EDU_ID;
			...
			for (int i = 0; i < array.size(); i++) {
				...
				//-----根据外键得到 学校，校区， 日期的信息 ------
				String url = getUrl(YingShuoConstant.YS_GET_UNIT_URL, "school_fkcode", fkCode);
	...
	public void saveDept() {
		...
		List<Unit> allSchool = SUtils.dt(unitRemoteService.findByRegionCode(YingShuoConstant.YS_REGION_CODE_VAL),
				new TR<List<Unit>>() {});
		Set<String> unitIds = EntityUtils.getSet(allSchool, Unit::getId);
		List<Dept> saveDepts = new ArrayList<Dept>();
		unitIds.forEach(c->{
			String url = getUrl(YingShuoConstant.YS_GET_DEPT_URL, "school_fkcode", getFkId(c));
			String cc = getDate (url,StringUtils.EMPTY,Boolean.TRUE);
			JSONArray array = Json.parseArray(cc);
			if(array != null){
				for (int i = 0; i < array.size(); i++) {
	...
	public void saveTeacher(String unitId) {
		...
		String cc = getDate (YingShuoConstant.YS_GET_TEACHER_URL,getBodyParam("school_fkcode", getFkId(unitId)));
		JSONArray array = Json.parseArray(cc);
		if(array != null){
			for (int i = 0; i < array.size(); i++) {
				JSONObject js = array.getJSONObject(i);
				String tid = doChangeId(js.getString("worker_fkcode"));
...
//					----------封装用户	
				if(StringUtils.isNotBlank(username)){
					...
					if(allUserMap != null && !allUserMap.isEmpty() && allUserMap.get(tid) != null){
						...
						user.setPassword(new PWD(YingShuoConstant.YS_DEFAULT_PASS_WORD_VALUE).encode());
						user.setOwnerType(User.OWNER_TYPE_TEACHER);
//					user.setRegionCode(YingShuoConstant.YS_REGION_CODE_VAL);
	...
	public void saveStudent(String unitId) {
		...
		List<User> allStuList = SUtils.dt(userRemoteService.findByUnitIds(unitId),new TR<List<User>>() {});
		...
		for (Clazz clazz : clazzList) {
			...
			String cc = getDate (YingShuoConstant.YS_GET_STUDENT_URL,getBodyParam(paramMap));
			JSONArray array = Json.parseArray(cc);
			if(array != null){
				for (int i = 0; i < array.size(); i++) {
					JSONObject js = array.getJSONObject(i);
					...
					Integer isDeleted = YingShuoConstant.DEFAULT_IS_DELETED_VALUE;
					...
					if(StringUtils.isBlank(studentCode)){
						studentCode = StringUtils.isBlank(identityCard) ? YingShuoConstant.YS_DEFAULT_STU_CODE 
								: identityCard;
					}
	...
	public void saveClass() {
		...
		if(CollectionUtils.isNotEmpty(allSchool)){
			for (School school : allSchool) {
				...
				String data = getDate (YingShuoConstant.YS_GET_CLAZZ_URL,getBodyParam("school_fkcode", getFkId(schoolId)));
				...
				if(array != null){
					for (int i = 0; i < array.size(); i++) {
						...
						if(StringUtils.isNotBlank(wpClassId)){
							...
							clazzId = doChangeId(js.getString("class_fkcode"));
						}
						...
						if(creyear == null){
							...
							startYear = dateFormat.format(creyear);
						}
...
//						String gradeName = gradeBefore + gradeYear;
	...
	public void saveCourse() {
		...
		setUids.forEach(c->{
			String cc = getDate (YingShuoConstant.YS_GET_COURSE_URL,getBodyParam("school_fkcode", getFkId(c)));
			JSONArray array = Json.parseArray(cc);
			if(array != null){
				for (int i = 0; i < array.size(); i++) {
					JSONObject js = array.getJSONObject(i);
	...
	public void saveBuilding() {
		...
		subIds.forEach(c->{
			String bodyParam = getBodyParam("campus_fkcode",StringUtils.substring(c, 13));
			String cc = getDate (YingShuoConstant.YS_GET_BUILDING_URL,bodyParam);
			JSONArray array = Json.parseArray(cc);
			if(array != null){
				for (int i = 0; i < array.size(); i++) {
					JSONObject build = array.getJSONObject(i);
					// 开始 ---------封装TeachBuilding
					String id = doChangeId(build.getString("building_fkcode"));
					...
					Integer isDeleted = YingShuoConstant.BOOLEAN_FALSE_VALUE;
					if(StringUtils.isNotBlank(build.getString("del_status")) && "2".equals(build.getString("del_status"))){
						isDeleted = YingShuoConstant.BOOLEAN_TRUE_VALUE;
					}
	...
	public void savePlace() {
		...
		buidIdSet.forEach(c->{
			String bodyParam = getBodyParam("building_fkcode",c);
			String cc = getDate (YingShuoConstant.YS_GET_PLACE_URL,bodyParam);
			JSONArray array = Json.parseArray(cc);
			if(array != null){
				for (int i = 0; i < array.size(); i++) {
					// 开始---------转换字段
					JSONObject js = array.getJSONObject(i);
					String teachPlaceId = doChangeId(js.getString("classroom_fkcode"));
					String placeName = js.getString("classroom_name");
					Integer isDeleted = YingShuoConstant.BOOLEAN_FALSE_VALUE;
					if(StringUtils.isNotBlank(js.getString("del_status")) && "2".equals(js.getString("del_status"))){
						isDeleted = YingShuoConstant.BOOLEAN_TRUE_VALUE;
					}
...
//					teachPlace.setPlaceType("4");// 默认是教室
	...
	public void saveSchoolSemester() {
		...
		schoolIds.forEach(c->{
			String bodyParam = getBodyParam("school_fkcode",c);
			String cc = getDate (YingShuoConstant.YS_GET_SCHOOL_SEMESTER_URL,bodyParam);
			JSONArray array = Json.parseArray(cc);
			if (array != null) {
				for (int i = 0; i < array.size(); i++) {
					// 开始---------转换字段
					...
					Date semesterBegin = js.getDate("start_date");
					Date semesterEnd = js.getDate("end_date");
					Integer isDeleted = YingShuoConstant.BOOLEAN_FALSE_VALUE;
					if(StringUtils.isNotBlank(js.getString("del_status")) && "3".equals(js.getString("del_status"))){
						isDeleted = YingShuoConstant.BOOLEAN_TRUE_VALUE;
					}
				    // 开始 ---------封装SchoolSemester 
	...
	private  String getAccessToken() {
		String accessToken = RedisUtils.get(YingShuoConstant.YS_TOKEN_REDIS_KEY);
		if (StringUtils.isBlank(accessToken)) {
			...
			if(StringUtils.isNotBlank(accessToken))
			    RedisUtils.set(YingShuoConstant.YS_TOKEN_REDIS_KEY, accessToken, (int)TimeUnit.MINUTES.toSeconds(29));
		}
		return accessToken;
	}
	
    private String getEduCode(){
    	HttpHeaders headers = new HttpHeaders();
		headers.setContentType(MediaType.APPLICATION_JSON_UTF8);
		JSONObject jsonObject = new JSONObject();
		jsonObject.put("userAccount", YingShuoConstant.YS_USER_NAME);
		jsonObject.put("pwd", YingShuoConstant.YS_USER_PASSWORD);
		HttpEntity<String> requestEntity = new HttpEntity<String>(jsonObject.toJSONString(), headers);
		ResponseEntity<String> responseBody = restTemplate.exchange(YingShuoConstant.YS_GET_FKCODE_URL, HttpMethod.POST, requestEntity, String.class);
    	JSONObject json = Json.parseObject(responseBody.getBody());
    	String data = getDate(json);
    	if(StringUtils.isNotBlank(data)){
    		JSONObject dataJson = Json.parseObject(data);
			try {
				return URLEncoder.encode(dataJson.getString("eduBureauFkCode"), "utf-8");
	...
	private ResponseEntity<String> getResponseEntity() {
		...
		headers.setContentType(MediaType.APPLICATION_JSON_UTF8);
		JSONObject jsonObject = new JSONObject();
		jsonObject.put("key", YingShuoConstant.YS_KEY_VALUE);
		jsonObject.put("security", YingShuoConstant.YS_SECURITY_VALUE);
		jsonObject.put("user_account", YingShuoConstant.YS_USER_NAME);
		HttpEntity<String> requestEntity = new HttpEntity<String>(jsonObject.toJSONString(), headers);
		ResponseEntity<String> responseBody = restTemplate.exchange(YingShuoConstant.YS_GET_TOKEN_URL, HttpMethod.POST, requestEntity, String.class);
		return responseBody;
	}
	...
	private String getDate(String url, String bodyParam, boolean isGet) {
		  HttpHeaders headers = new HttpHeaders();
		  headers.setContentType(MediaType.APPLICATION_JSON_UTF8);
		  headers.set(YingShuoConstant.YS_TOKEN_NAME, getAccessToken());
		if(isGet){
			...
			return jsonObject.getString(YingShuoConstant.YS_RESOLVE_DATA_NAME);
		}else{
			HttpEntity<String> requestEntity = new HttpEntity<String>(bodyParam, headers);
	...
	private String getBodyParam(String name, String value) {
		  return getBodyParam(null,name,value);
	}
	
	private String getBodyParam(Integer pageIndex, String name, String value){
		JSONObject jsonObject = new JSONObject();
		jsonObject.put("page_no", pageIndex== null ? 1 : pageIndex);
		jsonObject.put("page_size", YingShuoConstant.DEFAULT_PAGE_SIZE);
		jsonObject.put(name, getFkId(value));
	...
	private String getDate(JSONObject json){
		if(json != null){
			String code = json.getString(YingShuoConstant.YS_RESULT_CODE_NAME);
			if("0000".equals(code) || "ok".equalsIgnoreCase(code)){
				return json.getString(YingShuoConstant.YS_RESOLVE_DATA_NAME);
			}
		}
		return null;
	}
...
//				return URLEncoder.encode(dataJson.getString("records"), "utf-8");
	...
	private Integer doChargeIsUsing(String isUsing) {
		if(StringUtils.isNotBlank(isUsing) && "3".equals(isUsing)){
			return YingShuoConstant.BOOLEAN_FALSE_VALUE;
		}
		return YingShuoConstant.BOOLEAN_TRUE_VALUE;
	}
	
	private Integer doChargeIsDeleted(String isDeleted) {
		if(StringUtils.isNotBlank(isDeleted) && "3".equals(isDeleted)){
			return YingShuoConstant.BOOLEAN_TRUE_VALUE;
		}
		return YingShuoConstant.BOOLEAN_FALSE_VALUE;
	}
	...
	private Integer doChargeSemester(String semester) {
		if(StringUtils.isBlank(semester) || "第一学期".equals(semester)){
			return YingShuoConstant.FIRST_SEMESTER_VALUE;
		}
		return YingShuoConstant.SECTOND_SEMESTER_VALUE;
	}
	...
	private String doChargeBuildingType(String buildingType) {
		if(StringUtils.isBlank(buildingType) || "7".equals(buildingType) || "8".equals(buildingType)){
			return YingShuoConstant.YS_BUILDING_TYPE_OTHER;
		}
		return YingShuoConstant.YS_BUILDING_TYPE_TEACH;
	}
	...
	private void doInstallSectionTime(Semester semester,
		...
		String sectionTime = jsonObject.getString("basic_schedule");
		...
		for (int j = 0; j < sectionTimeArray.size(); j++) {
			...
			String beginTime = stjs.getString("start_time");
			String endTime = stjs.getString("end_time");
	...
	private void doInstallSubSchool(Map<String, SubSchool> subSchoolMap,
		...
		String schoolInfo = jsonObject.getString("basic_campus");
	...
	private void doInstallSchool(List<Unit> regionUnits,
		...
		String schoolInfo = jsonObject.getString("basic_schoolinfor");
		JSONArray schoolArray = Json.parseArray(schoolInfo);
		for (int j = 0; j < schoolArray.size(); j++) {
			JSONObject schoolJson = schoolArray.getJSONObject(j);
			...
			Date creationTime = schoolJson.getDate("setup_time"); 
			...
			if(allSchoolMap != null && !allSchoolMap.isEmpty() && allSchoolMap.get(unitId) != null){
				...
				school.setRegionCode(YingShuoConstant.YS_REGION_CODE_VAL);
	...
	private String doChargeUserName(String name) {
		return name = YingShuoConstant.YS_USERNAME_DEFEULT_BEFORE + name;
	}
