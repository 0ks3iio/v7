...
public class StuAttendanceRecordImportAction extends DataImportAction{
    ...
    @RequestMapping("/stuAttendanceRecord/import/main")
    public  String execute(ModelMap map, String classId,String gradeId){
        if(StringUtils.isNotBlank(classId)){
            ...
            // 业务名称
            map.put("businessName", "出勤记录");
            // 导入URL
            map.put("businessUrl", "/stutotality/stuAttendanceRecord/import?classId="+classId);
            // 导入模板
            //String searchDateStr=DateUtils.date2String(searchDate,"yyyy-MM-dd");
            map.put("templateDownloadUrl", "/stutotality/stuAttendanceRecord/template?acadyear="+acadyear+"&semester="+semester+"&classId="+classId);
            // 导入对象
        ...
        return "/stutotality/result/stuAttendanceRecordImport.ftl";
    }

    @Override
    public String getObjectName() {
        return "stuAttendanceRecordImport";
    }
    @Override
    ...
    @Override
    @RequestMapping("/stuAttendanceRecord/import")
    @ResponseBody
    public String dataImport(String filePath, String params) {
        ...
        String jsonMsg = stutotalityStuFinalService.attendRecordImport(getLoginInfo().getUnitId(), datas, acadyear, semester,classId);
        logger.info("导入结束......");
        return jsonMsg;
    }

    @Override
    @RequestMapping("/stuAttendanceRecord/template")
        ...
        for (StudentDto stu : list) {
            ...
            for(int t=0;t<5;t++){
                ...
                if(t==0){
                    ...
                    if(stu.getCasualLeave() == null ){
                        str = "";
                    }else {
                        str = stu.getCasualLeave()+"";
                    }
                }else if(t==3){
                    if(stu.getSickLeave() == null ){
                        str = "";
                    }else {
                        str = stu.getSickLeave()+"";
                    }
                }else {
                    if(stu.getOtherLeave() == null ){
                        str = "";
                    }else {
                        str = stu.getOtherLeave()+"";
                    }
                }
