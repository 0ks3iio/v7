...
<div id="itemShowDivId">
	<div class="box box-default">
		...
		<#if (bathNum>0)>
			<#list 1..bathNum as index>
				<label class="schedule-label schedule-label-${index}" data-filter="${index}"><i class="fa fa-plus"></i>${PCKC!}${index}</label>
			</#list>
		</#if>
		...
		<#assign edudays=7>
						...
						<td class="text-center edited" data-value="${tt}_${t}_${key!}">
						    <#if gkTimetableLimitArrangList?exists && (gkTimetableLimitArrangList?size>0)>
						         ...
						         <#if '${item.weekday}_${item.period}_${item.periodInterval}'=='${tt}_${t}_${key!}'>
							         <#if 'item.arrangId'!=''>						         
								         <label class="schedule-label schedule-label-${item.arrangId!} thin selected" data-label="${item.arrangId!}">${PCKC!}${item.arrangId!}</label>
								     </#if>
						         </#if>
						         </#list>
						    </#if>
			...
			<em>温馨提示：请先选择${PCKC!}或删除按钮进行操作，开班进行中不能修改（历史轮次也不能修改哦！）。</em>
		</div>
	</div>
</div>
...
function save(selectxyz,dataFilter,thisId){
	var id = '${arrangeId!}';
	var planId ='${planId!}';
	var label = '<label class="schedule-label schedule-label-'+dataFilter+' thin selected" data-label="'+dataFilter+'">${PCKC!}'+dataFilter+'</label>';
	$.ajax({
		url:'${request.contextPath}/gkelective/'+id+'/goClassTime/save',
		data: {'selectxyz':selectxyz,'dataFilter':dataFilter,'planId':planId},
		type:'post',
