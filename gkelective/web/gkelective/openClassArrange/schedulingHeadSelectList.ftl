<div class="box-body" style="padding-top:0px;padding-bottom:5px;">
	<#assign tableNum = 0>
	...
	<h3 <#if (tableNum>0)>class="showtableClass"</#if>>混合组合情况</h3>
	<table class="table table-bordered table-striped table-hover <#if (tableNum>0)>showtableClass</#if>"> 
		<tr>
			<th width="15%">组合</th>
			<td ><a href="javascript:" onclick="selectGroup('${gDto.subjectIds!}')">${gDto.conditionName!}</a></td>
		</tr>
		<tr>
			<th>总人数（班级数）</th>
			<td>${gDto.allNumber?default(0)}<#if (gDto.gkGroupClassList?exists) && (gDto.gkGroupClassList?size > 0)>(${gDto.gkGroupClassList?size})</#if></td>
		</tr>
		<tr>
			<th>剩余人数</th>
