...
//	}
    ...
    public String cutSingleArrange(@PathVariable String roundsId) {
    	try{
			...
			if(!ArrangeSingleSolver.isSolverIdRunning(roundsId) && round.getStep()>=GkElectveConstants.STEP_5){
				//直接下一步
	    		return success("no");
		    }
			ArrangeSingleSolver.stopSolver(roundsId);
    	}catch (Exception e) {
			e.printStackTrace();
    ...
    public String singleMain(@PathVariable String roundsId) {
    	...
    	try{
			...
			if(GkElectveConstants.TRUE_STR.equals(openClass)){
				//开AB
				mess=processSingleAB(true,subject_all,subject_all.length,gkArrange,round, sortType);
				...
				//只开选考DOTO
				mess=processSingleAB(false,subject_all,subjectNum,gkArrange,round, sortType);
			}
//			if(round.getStep() < GkElectveConstants.STEP_3){
//				round.setStep(GkElectveConstants.STEP_3);
//				gkRoundsService.save(round);
//			}
		}catch (Exception e) {
			e.printStackTrace();
			return error("保存失败！"+e.getMessage());
    ...
    public String  processSingleAB(boolean isAB,String[] subject_all, int subjectNum, final GkSubjectArrange gkArrange, final GkRounds round, String[] sortType){
...
//    	subjectNum=4;
    	//===
    	try{
    		if(GkRounds.OPENT_CLASS_TYPE_1.equals(round.getOpenClassType())){
        		//行政班排班  去除3+0
    			students = initSingleStudents2(true,isAB,subjectNum,subject_all,gkArrange,round.getId(),allstudents);
        	}else{
        		students = initSingleStudents2(false,isAB,subjectNum,subject_all,gkArrange,round.getId(),allstudents);
        	}
...
//    	int rangeSizeUp = 5;
//    	int rangeSizeDown = 5;
//    	int minCapacity = gkArrange.getClaNum() - rangeSizeDown;
//    	int maxCapacity = gkArrange.getClaNum() + rangeSizeUp;
//    	if (minCapacity < 2) {
//    	    minCapacity = gkArrange.getClaNum();
//    	}
//    	Map<String, ArrangeCapacityRange> subjectIdTypeCapacityRangeMap = new HashMap<String, ArrangeCapacityRange>();
//        ArrangeCapacityRange capacityRange = new ArrangeCapacityRange();
//        capacityRange.setMinCapacity(minCapacity);
//        capacityRange.setMaxCapacity(maxCapacity);
//        subjectIdTypeCapacityRangeMap.put("----------subjectId需要修改---------" + SelectionConstants.TYPE_A , capacityRange);
//        subjectIdTypeCapacityRangeMap.put("----------subjectId需要修改---------" + SelectionConstants.TYPE_B , capacityRange);
//     
        ...
        if(CollectionUtils.isNotEmpty(grouplist)){
        	for(GkGroupClass gg:grouplist){
                ...
                arrangeClassList.add(class1);
        	}
        }
        
        
        List<ArrangeStudent> arrangeStudentList = ArrangeDtoConverter.convertToArrangeStudent(students);
        ArrangeSingleSolver solver = new ArrangeSingleSolver(subjectNum, subjectNum, arrangeStudentList, arrangeClassList, round.getId());
        
        
        //ArrangeSingleSolver(int batchOrign, int batchMax, List<ArrangeStudent> studentList, List<ArrangeClass> arrangeClassList, String currentSolverId) {
        ...
        solver.addListener(new SolverListener() {
            ...
            public void solveFinished(Map<String, List<Room>>[] bottleArray, Map<String, Set<Integer>> classIdAdditionalSubjectIndexSetMap) {
                long start = System.currentTimeMillis();
                make(alldtoMap,gkArrange,round,bottleArray,subjectNameMap,classIdAdditionalSubjectIndexSetMap);
                long end = System.currentTimeMillis();
    ...
    private Map<String, ArrangeCapacityRange> findSingleMaxMinNum(String roundsId) {
    	...
    	if(CollectionUtils.isNotEmpty(condList)){
    		for(GkConditionDto dto:condList){
    			...
    			capacityRange.setClassNum(dto.getClaNum());
...
//	}
	...
	public String autoOpenClass(@PathVariable String roundsId,String subjectIds,int openNum,ModelMap map){
		try {
