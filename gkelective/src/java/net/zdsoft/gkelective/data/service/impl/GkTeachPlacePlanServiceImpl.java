	...
	public void savePlan(GkTeachPlacePlan plan) {
		...
		if(CollectionUtils.isNotEmpty(planList)){
			...
			for(String pId : planids){
				gkTimetableLimitArrangService.notUsing(planMap.get(pId).getAcadyear(),planMap.get(pId).getSemester(), pId);
			}
		}
		...
		if(GkRounds.OPENT_CLASS_TYPE_1.equals(rounds.getOpenClassType())){
			...
			for(GkGroupClass g:groupList){
				...
				clazz.setArtScienceType(1);//由于必填字段 默认1
...
//        	entityManager.clear();
	...
	public void saveItem(GkTeachPlacePlanDto dto,String type) {
		...
		if("2".equals(type)){
			...
			//判断不能排 根据批次所有班级  跟行政班与教学班排无关  场地分配 只负责单科  删除不需要批次的行政班设置
		...
		if(CollectionUtils.isNotEmpty(list)){
        	...
        	for(GkBatch b:list){
    			...
    			if(teachMap.containsKey(b.getTeachClassId())){
    				//只自动分配单科的
    				if(placeIdSet.size()>0 && BaseConstants.ZERO_GUID.equals(b.getGroupClassId())){
...
//		}
	...
	public void deleteByPlanId(String planId) {
        ...
        //删除限制
        gkTimetableLimitArrangService.deleteByRoundsId(plan.getAcadyear(), plan.getSemester(), plan.getId());
