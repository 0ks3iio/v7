...
<div class="row">
	<form id="planForm">
			    		...
			    		<#if subjectDtoList?exists && (subjectDtoList?size>0)>
			            	<#list subjectDtoList  as item>
			            		...
			            		<td>
			            		 ...
			            		 <input type="hidden" name="gkSubjectList[${item_index}].punchCard" value="${item.punchCard!}">
			            		${item_index+1}
			            		</td>
		...
		<div class="col-xs-12">
			<#if plan.step==0>	
			<em>说明：自动生成课表：根据7选3优先原则，对应上课时间内，行政班课程表会被清空，需要在之后重新安排行政班课表。</em>
			</#if>
			<div class="text-right">
			<#if plan.step==0>
				<a  href="javascript:" class="btn btn-blue  plan-save" ><#if isAdd>保存<#else>修改</#if></a>
				<a  href="javascript:" class="btn btn-blue  plan-nextStep-1" >自动生成课表</a>
			<#else>
