...
<div class="box-body groupArrangeList">
	...
	<div class="text-right">
		...
		<a class="btn btn-blue nextStep-btn singleBtnClass" <#if hasnowpaike>style="display:none"</#if> href="javascript:"><#if !canEdit || noData>下一步<#else>一键智能分班</#if></a>
...
$('.refresh-btn').on("click",function(){
	...
	showConfirm("重新安排将清除智能分班条件数据（包括走班开班结果数据），确定要重新安排开班吗？",options,refreshOpen,function(){});
});
...
function toPaiban(){
	...
	<#else>
	var url='${request.contextPath}/gkelectiveys/${roundsId!}/openClassArrange/singleResult/save';
	</#if>
	$.ajax({
		url:url,
		dataType : 'json',
...
function toPaiban2(){
	...
	<#else>
		var url='${request.contextPath}/gkelectiveys/${roundsId!}/openClassArrange/singleResult/check';
	</#if>
	$.ajax({
		url:url,
		dataType : 'json',
...
function cutPaiban(){
	...
	<#else>
		var url='${request.contextPath}/gkelectiveys/${roundsId!}/openClassArrange/singleResult/cut';
	</#if>
	$.ajax({
		url:url,
		dataType : 'json',
