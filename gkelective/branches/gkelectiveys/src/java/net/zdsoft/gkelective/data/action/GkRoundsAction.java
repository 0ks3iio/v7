...
public class GkRoundsAction extends BaseAction{
	...
	public String gKRoundsList(@PathVariable String arrangeId,String roundsId,ModelMap map){
		GkSubjectArrange gkSubArr = gkSubjectArrangeService.findArrangeById(arrangeId);
		if(gkSubArr==null){
			return errorFtl(map,"对应选课项目已经不存在");
		}
		List<GkRounds> allList=gKRoundsService.findBySubjectArrangeId(arrangeId,null);
		List<GkRounds> roundsList = new ArrayList<GkRounds>(); 
		if(StringUtils.isNotBlank(roundsId)){
			GkRounds rounds = gKRoundsService.findOne(roundsId);
			roundsList.add(rounds);
		}else{
			roundsList.addAll(allList);
		}
		...
		return "/gkelectiveys/arrangeRounds/roundsList.ftl";
	}
	...
	public String gKRoundsEdit(@PathVariable String arrangeId,ModelMap map){
		...
		map.put("roundsList", roundsList);
		map.put("arrangeId", arrangeId);
		return "/gkelectiveys/arrangeRounds/roundsEdit.ftl";
	}
