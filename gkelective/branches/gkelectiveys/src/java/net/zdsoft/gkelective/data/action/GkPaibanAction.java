...
//    }
    ...
    public void toTeachCls(GkRounds round,GkSubjectArrange gkArrange,String subjectId,String subtype,
		...
		if(BaseConstants.SUBJECT_TYPE_A.equals(subtype)){
			...
			int subNum = subjectNumAMap.get(subjectId);
			teachClass.setClassName(subjectNameMap.get(subjectId).getSubjectName()+"选考"+subNum+"班");
			...
			int subNum = subjectNumBMap.get(subjectId);
			//teachClass.setClassName(subjectNameMap.get(subjectId).getSubjectName()+subtype+subNum+"班");
			teachClass.setClassName(subjectNameMap.get(subjectId).getSubjectName()+"学考"+subNum+"班");
			subjectNumBMap.put(subjectId, subNum+1);
		}
		teachClassList.add(teachClass);
...
//	}
    ...
    @ResponseBody
	@RequestMapping("/openClassArrange/oneAutoOpenClass")
    @ControllerInfo(value = "全固定模式一键智能分班")
    public String oneAutoOpenClass(@PathVariable String roundsId, String copyRoundsId){
    	try {
    		if(StringUtils.isBlank(copyRoundsId)){
    			GkRounds gkRounds = gkRoundsService.findOne(roundsId);
    			List<GkRounds> gkRoundsList = gkRoundsService.findBySubjectArrangeId(gkRounds.getSubjectArrangeId(), null);
    			for (GkRounds item : gkRoundsList) {
					if(!item.getId().equals(gkRounds.getId()) && item.getOpenClassType().equals(gkRounds.getOpenClassType())
							&& item.getOpenTwo().equals(gkRounds.getOpenTwo()) && item.getStep()>=GkElectveConstants.STEP_4){
						copyRoundsId = item.getId();
						break;
					}
				}
    			if(StringUtils.isBlank(copyRoundsId)){
    				return error("一键智能分班失败！");
    			}
    		}
			List<GkGroupClass> groupClassList = gkGroupClassService.findByRoundsIdType(copyRoundsId, null);
			if(CollectionUtils.isNotEmpty(groupClassList)){
				GkGroupClassStu gkGroupClassStu = null;
				List<GkGroupClassStu> insertGroupClassStu = new ArrayList<GkGroupClassStu>();
				for (GkGroupClass gkGroupClass : groupClassList) {
					gkGroupClass.setId(UuidUtils.generateUuid());
					gkGroupClass.setRoundsId(roundsId);
					List<String> stuIdList = gkGroupClass.getStuIdList();
				...
				gkGroupClassService.saveGroup(groupClassList, insertGroupClassStu, roundsId);
			}
