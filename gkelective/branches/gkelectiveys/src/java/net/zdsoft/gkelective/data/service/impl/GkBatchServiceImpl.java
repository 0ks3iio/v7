...
//			}
	...
	public void deleteById(String batchId) {
		GkBatch findOne = gkBatchDao.findById(batchId).orElse(null);
		if(findOne == null){
	...
	@Override
	public void saveBatchsBySubjectIdType(
			String subjectType, 
			GkRounds round,
			List<GkBatch> gkBatchList,
			List<GkTeachClassStore> teachClassList,
			List<GkTeachClassStuStore> teachClassStuList,
			List<GkTeachClassEx> exList, boolean isUpdateStep) {
		//将之前轮次的教学班毕业---在新建轮次的时候加排除啦
        List<GkBatch> oldGkBatchList = null;
        Integer step = null;
		//清除单科数据批次 教学班
        ...
        if (CollectionUtils.isNotEmpty(oldGkBatchList)) {
            for (GkBatch item : oldGkBatchList) {
            	if(subjectType.equals(item.getClassType())){
            		oldteachIds.add(item.getTeachClassId());
                    batchIds.add(item.getId());
            	}
            }
		...
		if(CollectionUtils.isNotEmpty(gkBatchList)){
			for(int i = 0; i < gkBatchList.size(); i++) {  
				entityManager.persist(gkBatchList.get(i));  
	            if(i % 30== 0) {  
	            	entityManager.flush();  
	            	entityManager.clear();  
	            }  
		...
		if(CollectionUtils.isNotEmpty(teachClassList)){
			for(int i = 0; i < teachClassList.size(); i++) {  
				entityManager.persist(teachClassList.get(i));  
	            if(i % 30== 0) {  
	            	entityManager.flush();  
	            	entityManager.clear();  
	            }  
		...
		if(CollectionUtils.isNotEmpty(teachClassStuList)){
			for(int i = 0; i < teachClassStuList.size(); i++) {  
				entityManager.persist(teachClassStuList.get(i));  
	            if(i % 30== 0) {  
	            	entityManager.flush();  
	            	entityManager.clear();  
	            }  
