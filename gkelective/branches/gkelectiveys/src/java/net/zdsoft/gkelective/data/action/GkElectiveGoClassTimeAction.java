...
public class GkElectiveGoClassTimeAction extends BaseAction{
    ...
    public String showIndex(@PathVariable String arrangeId,String planId,ModelMap map) {
		...
		List<GkTimetableLimitArrang> gkTimetableLimitArrangList = gkTimetableLimitArrangService.findGkTimetableLimitArrangList(gaent.getUnitId(), plan.getAcadyear(),plan.getSemester(), plan.getId(), GkElectveConstants.LIMIT_ARRANG_TYPE);
    	...
    	int bathNumA = rent.getBatchCountA()==null?0:rent.getBatchCountA();
    	int bathNumB = rent.getBatchCountB()==null?0:rent.getBatchCountB();
...
//    		bathNum=3;
    	...
    	map.put("bathNumA", bathNumA);
    	map.put("bathNumB", bathNumB);
//    	map.put("bathNum", bathNum);
		...
		map.put("gaent", gaent);
		map.put("plan", plan);
		return "/gkelectiveys/goClassTime/goClassTimeIndex.ftl";
	}
