									...
									<tbody class="interList">
									    <#if openInterfaceDtos?exists&&openInterfaceDtos?size gt 0>
          	                               <#list openInterfaceDtos as dto>
									            <tr onclick="chooseInter('${dto.resultType!}','${dto.type!}',this)" data-action="select">
									                <td width="20">
														<label class="pos-rel">
...
var map_=getMap();
var entityIds;
function chooseInter(resultType,type,event){
	selType = type;
	doUpdateInter(resultType,type,event);
	inter = $(event).children('td').last().html();
}

function doUpdateInter(resultType,type,event){
...
function getEntityList(resultType){
	$.ajax({
        url:"${request.contextPath}/data/manage/showEntityList?type="+resultType,
        data:{},
        async: false,
        dataType:'json',
...
function getEndResult(type){
   ...
   var htmlResult = "";
   if(selEle.length > 0) {
	   htmlResult += '<tbody class = "trmg" id = "inter-'+type+'" data-type="'+type+'" ><tr><td rowspan="'+rowspan+'" width="85">' + inter + '</td></tr>';
