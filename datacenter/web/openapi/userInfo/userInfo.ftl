              ...
              if(browser.ie){
               ...
               $('.userInfoClass').on('click','#eyeChange i.fa',function(){
                ...
                $('.userInfoClass').on('blur','#oldPassword',function(){
                  ...
                  $.ajax({
                    url:'${request.contextPath}/developer/regist/valid/passwordVail/'+$(this).val(),
                    type:'GET',
                    dataType:'json',
                ...
                $('.userInfoClass').on('click','#updatePassword',function(){
                  ...
                  if($(this).parents('.mpwdDiv').find('.has-error').length==0){
                    $.post('${request.contextPath}/developer/modifyPwd',{'oldPwd':$('#oldPassword').val(),'newPwd':$('#newPwd').val()},function(result){
