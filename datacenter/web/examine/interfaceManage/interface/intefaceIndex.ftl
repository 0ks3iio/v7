...
$('#addInterface').on('click', function(e){
	$('.layer-interface-detailedId').load("${request.contextPath}/datacenter/examine/interface/editInterface",function(){			
	layer.open({
				type: 1,
				shade: .5,
				title: '新增接口',
				...
				yes:function(index,layero){
				   saveInterface("${request.contextPath}/datacenter/examine/interface/saveInterface");
	            },
...
function startInte(){
  ...
  updateInterface("${request.contextPath}/datacenter/examine/interface/isUsingInterface?interfaceId="+inteId+"&isUsing=1");
}  

function stopInte(){
  var inteId = $(this).parents('tr').data('interid');
  updateInterface("${request.contextPath}/datacenter/examine/interface/isUsingInterface?interfaceId="+inteId+"&isUsing=0");
} 

function modifyDetails(){
  var inteId = $(this).parents('tr').data('interid');
  $("#tabList").load("${request.contextPath}/datacenter/examine/interface/param/showIndex?interfaceId="+inteId);
} 
   
function modifyInte(){
  var interid = $(this).parents('tr').data('interid');
  $('.layer-interface-detailedId').load("${request.contextPath}/datacenter/examine/interface/editInterface?interfaceId="+interid,function(){			
  layer.open({
			type: 1,
			shade: .5,
			title: '修改参数',
...
function showInterfaceList(){
  ...
  $(".table-container").load("${request.contextPath}/datacenter/examine/interface/showInterfaces?typeName="+interfaceName+"&isUsing="+isUsing+"&dataType="+dataType);
}
...
function deleteInte(){
	...
	function(){
		$.ajax({
            url:"${request.contextPath}/datacenter/examine/interface/delInterface?interfaceId="+inteId,
            data:{},
            dataType:'json',
            contentType:'application/json',
            type:'GET',
