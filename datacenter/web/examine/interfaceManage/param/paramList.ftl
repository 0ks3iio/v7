...
$('.delete').on('click', function(e){
			...
			function(){
				$.ajax({
		            url:"${request.contextPath}/datacenter/examine/interface/param/delParam?paramId="+paramId,
		            data:{},
		            dataType:'json',
...
$('.modify').on('click', function(e){
	var paramId = $(this).parents('tr').data('paramid');
	$('.layer-param-detailedId').load("${request.contextPath}/datacenter/examine/interface/param/editParam?paramId="+paramId,function(){			
	layer.open({
				type: 1,
				shade: .5,
				title: '修改参数',
