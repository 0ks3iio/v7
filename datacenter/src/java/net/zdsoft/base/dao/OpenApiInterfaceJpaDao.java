...
public interface OpenApiInterfaceJpaDao extends BaseJpaRepositoryDao<OpenApiInterface, String> {
	...
	@Query("select distinct type from OpenApiInterface where isUsing = 1")
	public List<String> findDistinctType();

	@Query("select distinct type from OpenApiInterface where dataType = ?1" + SQL_AFTER)
	...
	@Query("From OpenApiInterface where dataType = (?1)" +  SQL_AFTER)
