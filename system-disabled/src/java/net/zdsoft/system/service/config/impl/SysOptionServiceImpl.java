...
public class SysOptionServiceImpl extends BaseServiceImpl<SysOption, String> implements SysOptionService {
    ...
    private void doCacheValueSave(final String optionCode, final SysOption save) {
        ...
        if (value != null) {
            RedisUtils.setObject(KEY + optionCode + ".value", value);
        }
    }
    ...
    public SysOption findOneByOptionCode(String optionCode) {
    	if (StringUtils.isBlank(optionCode))
			return null;
    	SysOption ini = LocalCacheUtils.getValue("SysOption", optionCode);
		if(ini == null) {
			List<SysOption> sysOptions = sysOptionDao.findByOptionCode(optionCode);
			if(CollectionUtils.isNotEmpty(sysOptions)) {
				ini = sysOptions.get(0);
			}
			if(ini != null) {
				LocalCacheUtils.putValue("SysOption", optionCode, ini);
			}
		}
		return ini;
    	
//        List<SysOption> findByOptionCode = sysOptionDao.findByOptionCode(optionCode);
//        if (findByOptionCode != null && findByOptionCode.size() > 0) {
//            return findByOptionCode.get(0);
//        }
//        return null;
    }
    ...
    public void updateNowValueByCode(String nowValue, String code) {
        ...
        RedisUtils.setObject(KEY + code + ".value", nowValue);
    }
