...
<script>
function queryMList(roomNo){
     var examId = '${examId!}';
     var subId = $('#subjectId').val();
     var url = "${request.contextPath}/teaexam/siteSet/query/mPasteList?examId="+examId+"&subId="+subId+"&roomNo="+roomNo;
     $('#siteTabDiv').load(url);
}
...
function onBatchPrint(){
    var examId = '${examId!}';
    var subId = $('#subjectId').val();
    document.getElementById('batchPrintId').src="${request.contextPath}/teaexam/siteSet/query/mBatchPrint?examId="+examId+"&subId="+subId+"&batchId=${batchId!}";
}
...
function doBatchPrint(){
	...
	//var subName = window.frames["batchPrintId"].document.getElementById("subName").value;
	...
	if (batchIdLeftVal == "") {
		if (window.frames["batchPrintId"].document.getElementById("doNotPrint").value == "0") {
            ...
            LODOP.ADD_PRINT_TEXT(10,0,"95%",30,'考试名称：'+examName+'  考点：'+unitName+'  考生'+listSize+'人    第#页/共&页');
			LODOP.PRINT();
			//LODOP.PREVIEW();//打印预览
	     	showMsgSuccess("打印成功！");
		} 
		...
		LODOP.PRINT();
		//LODOP.PREVIEW();//打印预览
		document.getElementById('batchPrintId').src="${request.contextPath}/teaexam/siteSet/query/mBatchPrint?examId=${examId!}&subId="+subId+"&batchId="+batchIdLeftVal;
	}
}
...
function doprint(){
	...
	//LODOP.ADD_PRINT_TEXT('20', '250','400', '50', '考试名称：${examName!}  考点：${roomNoLocationIdMap[roomNo!]!}  考场：${roomNo!}  考生${regList2?size}人');
	//设置打印内容，getPrintContent函数是LodopFuncs中增加的一个函数，参数传入jQuery对象，第二个参数表示要忽略的样式名，可以不写（主要是有些样式与打印冲突，如prt-report-wrap，这个价了后，后面的内容就不会显示）
	...
	LODOP.SET_PRINT_STYLE("Alignment", 2);
	LODOP.SET_PRINT_STYLE("FontSize", 11.5);
    LODOP.ADD_PRINT_TEXT(10,0,"95%",30,"考试名称：${examName!}  考点：${roomNoLocationIdMap[roomNo!]!}  考生${regList2?size}人    第#页/共&页");
	LODOP.PREVIEW();//打印预览
}
</script>