...
<div class="box box-default">
	<div class="box-body">
		<ul class="nav nav-tabs nav-tabs-1" role="tablist">
			<li class="active" role="presentation"><a href="javascript:;" role="tab" data-toggle="tab" onclick="showSubStu()">已提交（${subNum}）</a></li>
			<li role="presentation"><a href="javascript:;" role="tab" data-toggle="tab" onclick="showNoSubStu()">未提交（${noSubNum}）</a></li>
		</ul>
				...
				<div class="filter">
					<div class="filter-item">
						...
						<div class="filter-content">
							<select name="gradeId" id="gradeId" class="form-control" onchange="findByCondition()" style="width:120px">
					...
					<div class="filter-item">
						...
						<div class="filter-content">
							<select name="classId" id="classId" class="form-control" onchange="findByCondition()" style="width:120px">
							...
							<#if stulist?? && stulist?size gt 0>
								<#list stulist as stu>
									<tr>
										...
										<td>${stu.className!}</td>
										<td><a href="javascript:" onclick="toResultDetail('${stu.id!}')">查看</a></td>
									</tr>
								</#list>
							</#if>
							</tbody>
...
function findByCondition(){
	...
	var url =  '${request.contextPath}/evaluate/project/showStuSubList/page?projectId=${projectId!}&acadyear=${acadyear!}&semester=${semester!}&gradeId='+gradeId+'&classId='+classId+'&selectType='+selectType+'&selectObj='+selectObj;
	$(".model-div").load(url);
}
...
function toResultDetail(studentId){
	var gradeId = $('#gradeId').val();
	var classId = $('#classId').val();
	var selectObj = $('#selectObj').val();
	var selectType = $('#selectType').val();
	var url =  '${request.contextPath}/evaluate/stu/edit?projectId=${projectId!}&acadyear=${acadyear!}&semester=${semester!}&gradeId='+gradeId+'&classId='+classId+'&selectType='+selectType+'&selectObj='+selectObj+"&studentId="+studentId;
	$(".model-div").load(url);
}
</script>